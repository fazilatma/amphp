...
class Scraper_Auto_Shop_Plugin {
	...
	public static function get_default_settings() {
		return array(
			...
			'set_wc_shop_to_fallback'     => false, // v13.3.20: پیش‌فرض خاموش — پشتیبان جای ویترین React را نگیرد
	...
	public static function send_message_to_messengers( $message_text, $settings = null, $media = array() ) {
		...
		foreach ( $messengers as $key => $m ) {
			...
			// v13.3.20: فوروارد چندرسانه‌ای (عکس/ویدیو/گیف/فایل/لینک)
	...
	/**
	 * v13.3.20: ارسال یک آیتم رسانه به یک پیام‌رسان.
	...
	/**
	 * v13.3.20: تشخیص نوع رسانه از URL یا MIME.
	 */
	...
	/**
	 * v13.3.20: ذخیره پیوست آپلود‌شدهٔ چت پشتیبانی و ساخت آیتم رسانه.
	...
	/**
	 * v13.3.20: استخراج لینک‌های داخل متن پیام مشتری به‌عنوان آیتم رسانه.
	 */
...
"
	...
	public static function ensure_fallback_shop_page( $assign_wc_shop = null ) {
		...
		if ( $page_id > 0 ) {
			...
			// v13.3.20: برگهٔ پشتیبان ≠ صفحهٔ فروشگاه اصلی — shop_page_id را روی پشتیبان ننویس
			$settings['native_fallback_page_id'] = $page_id;
			update_option( self::OPTION_NAME, $settings );

			/* v13.3.20: فقط وقتی صریحاً خواسته شده و takeover React خاموش است،
	...
	/**
	 * v13.3.20: اگر فروشگاه ووکامرس اشتباهاً روی برگهٔ «پشتیبان» نشسته و
	...
	/**
	 * v13.3.20: برگهٔ اصلی فروشگاه برای takeover React (جدا از «— پشتیبان»).
	 *
	 * @param array $settings
	...
	public static function maybe_takeover_shop_template( $template ) {
		...
		if ( $should_takeover ) {
			// v13.3.20: ویترین اصلی = همیشه React bare، مگر قالب صریحاً native-wp انتخاب شده باشد.
			// enable_native_wp_template فقط برگهٔ پشتیبان را فعال می‌کند — takeover را native نکن.
			...
			// اگر صفحهٔ فروشگاه ووکامرس اشتباهاً همان برگهٔ پشتیبان است، باز هم React را نشان بده
	...
	public static function render_standalone_shop_page() {
		...
		header( 'X-AMPHP-Storefront: bare-v13.3.20' );
			...
			if ( ! empty( $new_settings['enable_native_wp_template'] ) || ! empty( $new_settings['auto_create_fallback_page'] ) ) {
				try {
					...
					if ( ! empty( $fb_res['id'] ) ) {
						...
						// v13.3.20: shop_page_id را با پشتیبان یکی نکن
						update_option( self::OPTION_NAME, $new_settings );
					}
			...
			<form method="post" action="" id="scraperAdminForm">
				...
				<div id="tab-storefront" class="scraper-tab-panel active">
					<div class="admin-card">
						...
						<!-- v13.3.20 قالب بومی وردپرس + برگه پشتیبان -->
						<div style="margin:20px 0 24px; background:linear-gradient(135deg,#f8fafc,#eff6ff); border:1px solid #93c5fd; border-radius:14px; padding:20px;">
							...
							<p style="margin:0 0 14px; padding:10px 12px; background:#fef3c7; border:1px solid #f59e0b; border-radius:10px; color:#92400e; font-size:0.82rem; font-weight:700; line-height:1.7;">
								⚠️ اگر با رفرش فقط صفحهٔ «… — پشتیبان» می‌بینید: قالب ویترین را روی native نگذارید، تیک «صفحه فروشگاه ووکامرس = پشتیبان» را خاموش کنید، ذخیره کنید. از v13.3.20 افزونه خودش پشتیبان را از فروشگاه اصلی جدا می‌کند.
							</p>
							...
							<label style="display:flex; align-items:center; gap:10px; margin-bottom:12px; font-weight:700; color:#334155;">
								...
								فقط اگر فروشگاه ووکامرس خالی است و ویترین React خاموش است، پشتیبان را صفحهٔ فروشگاه ووکامرس کن (با React روشن توصیه نمی‌شود)
							</label>
							<?php
