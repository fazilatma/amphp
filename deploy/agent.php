...
class Scraper_Auto_Shop_Plugin {
	...
	public static function get_default_settings() {
		return array(
			...
			'takeover_front_page'         => true, // v13.3.30: صفحه نخست هم ویترین React
			...
			'set_wc_shop_to_fallback'     => false, // v13.3.30: پیش‌فرض خاموش — پشتیبان جای ویترین React را نگیرد
	...
	public static function get_settings() {
		...
		/* v13.3.30: ویترین React روی دامنهٔ اصلی (صفحهٔ خانه) — نه فقط /shop یا ?amphp_shop=1 */
	...
	/**
	 * v13.3.30: nonce اتصال مستقیم — هم scraper_woo_bridge و هم scraper_shop_admin_nonce
	...
	public static function send_message_to_messengers( $message_text, $settings = null, $media = array() ) {
		...
		foreach ( $messengers as $key => $m ) {
			...
			// v13.3.30: فوروارد چندرسانه‌ای (عکس/ویدیو/گیف/فایل/لینک)
	...
	/**
	 * v13.3.30: ارسال یک آیتم رسانه به یک پیام‌رسان.
	...
	/**
	 * v13.3.30: تشخیص نوع رسانه از URL یا MIME.
	 */
	...
	/**
	 * v13.3.30: ذخیره پیوست آپلود‌شدهٔ چت پشتیبانی و ساخت آیتم رسانه.
	...
	/**
	 * v13.3.30: استخراج لینک‌های داخل متن پیام مشتری به‌عنوان آیتم رسانه.
	 */
...
"
	...
	public static function maybe_load_native_page_template( $template ) {
		...
		/* v13.3.30: صفحهٔ خانه همیشه React — قالب native را اینجا بارگذاری نکن */
		$settings = self::get_settings();
	...
	public static function ensure_fallback_shop_page( $assign_wc_shop = null ) {
		...
		if ( $page_id > 0 ) {
			...
			// v13.3.30: برگهٔ پشتیبان ≠ صفحهٔ فروشگاه اصلی — shop_page_id را روی پشتیبان ننویس
			$settings['native_fallback_page_id'] = $page_id;
			update_option( self::OPTION_NAME, $settings );

			/* v13.3.30: فقط وقتی صریحاً خواسته شده و takeover React خاموش است،
	...
	/**
	 * v13.3.30: ذخیرهٔ خودکار تنظیمات ادمین (AJAX) بدون رفرش صفحه.
	 */
	...
	/**
	 * v13.3.30: استخراج آرایهٔ تنظیمات از درخواست POST (ذخیره دستی و خودکار مشترک).
	 *
	 * @return array
	 */
	public static function build_settings_from_request() {
		...
		/* v13.3.30: خانهٔ سایت همیشه React وقتی takeover روشن است */
	...
	/**
	 * v13.3.30: اگر فروشگاه ووکامرس اشتباهاً روی برگهٔ «پشتیبان» نشسته و
	...
	/**
	 * v13.3.30: برگهٔ اصلی فروشگاه برای takeover React (جدا از «— پشتیبان»).
	 *
	 * @param array $settings
	...
	/**
	 * v13.3.30: آیا این درخواست باید ویترین React bare (دیجی‌کالا و …) باشد؟
	 */
	
	/**
	 * v13.3.30: نوار/منوی خود پیشخوان وردپرس — فقط دسکتاپ sticky؛ موبایل دست‌نخورده
...
/* ——— پنل افزونه v13.3.30: فشرده، بدون اسکرول افقی، موبایل‌دوست ——— */
...
@media screen and (max-width: 480px) {
  ...
  .amphp-admin-hero .amphp-hero-badge{font-size:0.68rem!important}
	...
	/**
	 * v13.3.30: آیا درخواست فعلی صفحهٔ اصلی دامنه است؟ (/, front page، posts home)
	...
	public static function should_render_react_storefront( $settings = null ) {
		...
		// v13.3.30: صفحهٔ اصلی دامنه (/, front page, blog home) → React bare
		if ( self::is_site_home_request() ) {
			return true;
		}
		if ( ( function_exists( 'is_shop' ) && is_shop() ) || ( function_exists( 'is_product_taxonomy' ) && is_product_taxonomy() ) ) {
		...
		$fb_id = intval( $settings['native_fallback_page_id'] ?? get_option( 'scraper_native_fallback_page_id', 0 ) );
		/* اگر صفحهٔ اصلی همان برگهٔ پشتیبان باشد، React روی خانه اولویت دارد (v13.3.30) */
	...
	public static function render_standalone_shop_page() {
		...
		header( 'X-AMPHP-Storefront: bare-v13.3.30' );
...
public static function get_embedded_storefront_assets() {
		...
		// Inline fallback baked at build time (v13.3.30) — single-file deploy.
		$cache = array(
			'storefront.js'  => array(
	...
	/**
	 * v13.3.30: Boot سبک برای چت پشتیبانی هوشمند (صفحه بومی / پشتیبان).
	 *
	 * @return array
	 */
	...
	/**
	 * v13.3.30: چاپ ویجت چت پشتیبانی هوشمند روی قالب بومی / هر صفحهٔ غیر React.
	...
	public static function print_native_support_chat_widget() {
		...
		echo "\n<!-- AMPHP support chat (native) v13.3.30 -->\n";
	...
	/**
	 * v13.3.30: wp_footer hook — چت روی قالب بومی.
	 */
	 ...
	 */
			...
			if ( ! empty( $new_settings['enable_native_wp_template'] ) || ! empty( $new_settings['auto_create_fallback_page'] ) ) {
				try {
					...
					if ( ! empty( $fb_res['id'] ) ) {
						...
						// v13.3.30: shop_page_id را با پشتیبان یکی نکن
						update_option( self::OPTION_NAME, $new_settings );
					}
					self::maybe_detach_fallback_from_primary_shop();
				} catch ( \Throwable $e ) { /* ignore */ }
			}
			// v13.3.30: با React روشن، primary بساز و پشتیبان را از /shop جدا کن
			if ( ! empty( $new_settings['enable_shop_takeover'] ) ) {
				try {
		...
		<div class="wrap scraper-admin-dashboard">
			...
			<!-- Header Title Area v13.3.30 -->
			<div class="amphp-admin-hero">
				<div style="min-width:0;flex:1 1 220px;">
					<div class="amphp-hero-badge">⚡ پنل فروشگاه · v13.3.30</div>
			...
			<form method="post" action="" id="scraperAdminForm">
				...
				<div id="tab-storefront" class="scraper-tab-panel active">
					<div class="admin-card">
						...
						<?php
						// v13.3.30: وضعیت ویترین React در برابر پشتیبان — راهنمای پیدا کردن تیک‌ها
						...
						/* v13.3.30: لینک اصلی ویترین = دامنهٔ خانه */
						...
						<!-- v13.3.30: کلید روشن/خاموش ووکامرس=پشتیبان — بدون اتکا به ظاهر چک‌باکس وردپرس -->
...
<!-- Visual eCommerce Storefront Templates Selector -->
							...
							<tr>
								...
								<td>
									<div style="display:flex;align-items:flex-start;gap:10px;padding:12px 14px;background:#ecfdf5;border:2px solid #10b981;border-radius:12px;font-weight:800;">
										...
										<span style="font-size:0.82rem;font-weight:700;color:#047857;">از v13.3.30 با روشن بودن «ویترین React» صفحهٔ خانه خودکار React است — نه آدرس فرعی.</span></span>
						...
						<!-- v13.3.30 قالب بومی وردپرس + برگه پشتیبان -->
						<div style="margin:20px 0 24px; background:linear-gradient(135deg,#f8fafc,#eff6ff); border:1px solid #93c5fd; border-radius:14px; padding:20px;">
							...
							<p style="margin:0 0 14px; padding:10px 12px; background:#fef3c7; border:1px solid #f59e0b; border-radius:10px; color:#92400e; font-size:0.82rem; font-weight:700; line-height:1.7;">
								⚠️ اگر با رفرش فقط صفحهٔ «… — پشتیبان» می‌بینید: قالب ویترین را روی native نگذارید، تیک «صفحه فروشگاه ووکامرس = پشتیبان» را خاموش کنید، ذخیره کنید. از v13.3.30 افزونه خودش پشتیبان را از فروشگاه اصلی جدا می‌کند.
							</p>
...
<!-- Custom Checkout Settings -->
			...
			<script>
			(function(){
			  // v13.3.30: ذخیرهٔ خودکار تنظیمات + وضعیت
...
$('#scraperAdminTabs .scraper-tab-link').on('click', function(e){
			...
			/* v13.3.30: nonce درست برای پل ووکامرس (قبلاً mismatch → 403/-1) */
