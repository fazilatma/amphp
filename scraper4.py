...
function openAITestModal(){if(!aiTestJobData){loadAITestJobs().then(()=>{if(aiTestJobData)openAITestModal()});return}$('aiTestModal').classList.add('open');document.body.style.overflow='hidden';if(window.innerWidth<720){let f=$('aiLabFilters');if(f)f.open=false;let ps=$('aiResultPageSize');if(ps)ps.value='25';aiResultPage=1}renderAITestModal()}
