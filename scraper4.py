...
def basalam_api_request(method: str, path: str, *, params: Optional[dict[str,Any]]=None, json_data: Any=None, data: Any=None, files: Any=None) -> Any:
    """Direct SalamAPI REST transport; independent from basalam-sdk."""
    cfg=load_data().get("basalam",{});token=normalize_basalam_token(cfg.get("token"));base=public_http_url(clean_text(cfg.get("api_base_url")) or "https://openapi.basalam.com").rstrip("/");target=base+"/"+path.lstrip("/")
    if params:target=requests.Request("GET",target,params=params).prepare().url
    authorization="Bearer "+token;headers={"Authorization":authorization,"Accept":"application/json","User-Agent":"Scraper4-REST/"+APP_VERSION};transport=clean_text(cfg.get("api_mode","relay"));request_url=target
    if transport=="relay":
        relay=public_http_url(clean_text(cfg.get("relay_url")) or "https://proxy.fazilat-ma.workers.dev");request_url=relay+("&" if "?" in relay else "?")+urlencode({"url":target});headers.update({"X-Proxy-UA":headers["User-Agent"],"X-Proxy-Authorization":authorization,"X-Upstream-Authorization":authorization,"X-Proxy-Method":method.upper()})
        if cfg.get("worker_key"):headers["X-Proxy-Key"]=str(cfg["worker_key"])
    def send(url: str, hdr: dict[str,str]) -> requests.Response:
        return requests.request(method,url,headers=hdr,json=json_data,data=data,files=files,timeout=60,allow_redirects=True)
    try:response=send(request_url,headers)
    except requests.RequestException as exc:raise FetchError(f"REST API باسلام از مسیر {transport} قابل دسترسی نیست: {exc}") from exc
    if not response.ok and transport=="relay" and response.status_code in {401,403}:
        relay_error=f"HTTP {response.status_code}: {clean_text(response.text)[:350]}";rewind_upload_files(files)
        try:direct=send(target,{k:v for k,v in headers.items() if not k.startswith("X-Proxy-") and not k.startswith("X-Upstream-")})
        except requests.RequestException as exc:raise FetchError(f"REST API: Worker احراز هویت را عبور نداد ({relay_error}) و اتصال مستقیم نیز شکست خورد: {exc}") from exc
        if direct.ok:response=direct;transport="direct-fallback"
        else:raise FetchError(f"REST API: رله={relay_error}؛ مستقیم=HTTP {direct.status_code}: {clean_text(direct.text)[:350]}. Worker اختصاصی باید Authorization، method و body را منتقل کند")
    if not response.ok:raise FetchError(f"Basalam REST {transport} HTTP {response.status_code}: {clean_text(response.text)[:700]}")
