...
def ensure_basalam_sdk() -> bool:
    ...
    try:
        ...
        cmd=[python_bin,"-m","pip","install","--disable-pip-version-check","--no-cache-dir"]
        if not inside_app_venv:
            os.makedirs(LOCAL_DEPS_DIR,mode=0o700,exist_ok=True);cmd.extend(["--target",LOCAL_DEPS_DIR])
        cmd.append("basalam-sdk")
