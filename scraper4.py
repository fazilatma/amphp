...
def api_picker_preview():
    ...
    if result is None or spa:
        try:result=render_playwright(url,fetcher.timeout,scrolls)
        except FetchError as exc:
            errors.append(f"playwright: {exc}")
            if result is None and render in {"auto","browser"} and VPS_MODE:
                try:result=render_selenium(url,fetcher.timeout,scrolls)
                except FetchError as exc2:errors.append(f"selenium: {exc2}")
    elif render in {"auto","browser"} and result is None and VPS_MODE:
        try:result=render_selenium(url,fetcher.timeout,scrolls)
...
<div class="chrome-title">اسکرپر <small id="chromeVer">v10.135</small></div>
...
function prettyProfile(name){let n=String(name||'');try{n=decodeURIComponent(n)}catch(e){}n=n.replace(/^https?:\/\//i,'').replace(/^www\./i,'');n=n.replace(/_([0-9A-Fa-f]{2})/g,(_,h)=>{try{return decodeURIComponent('%'+h)}catch(e){return ' '}});n=n.replace(/[_.\-]+/g,' · ');n=n.replace(/\s+/g,' ').trim();return n||String(name||'پروفایل')}
function renderProfiles(){const entries=Object.entries(profiles);$('profileList').innerHTML=entries.map(([n,c])=>`<div class="card profile-card" onclick='loadProfile(${JSON.stringify(n)},true,true)'><b>${n===activeProfile?'● ':''}${esc(prettyProfile(n))}</b><br><small dir="ltr">${esc(c.url)}</small><div class="actions"><button class="gray" onclick='event.stopPropagation();delProfile(${JSON.stringify(n)})'>حذف</button></div></div>`).join('')||'<div class="note">هنوز پروفایلی نیست.</div>';$('profileSelect').innerHTML='<option value="">— پروفایل جدید —</option>'+entries.map(([n])=>`<option value="${esc(n)}">${esc(prettyProfile(n))}</option>`).join('');$('profileSelect').value=activeProfile||'';if($('dispatchProfile')){$('dispatchProfile').innerHTML=entries.map(([n,c])=>`<option value="${esc(n)}">${esc(prettyProfile(n))} — ${toFa(c.saved_products?.length||0)} محصول</option>`).join('')||'<option value="">پروفایلی موجود نیست</option>';$('dispatchProfile').value=activeProfile&&profiles[activeProfile]?activeProfile:(entries[0]?.[0]||'')}if($('dispatchProfileBasalam')){$('dispatchProfileBasalam').innerHTML=entries.map(([n,c])=>`<option value="${esc(n)}">${esc(prettyProfile(n))} — ${toFa(c.saved_products?.length||0)} محصول</option>`).join('')||'<option value="">پروفایلی موجود نیست</option>';$('dispatchProfileBasalam').value=activeProfile&&profiles[activeProfile]?activeProfile:(entries[0]?.[0]||'')}}
function updateActiveProfileUI(){let p=profiles[activeProfile];$('profileSelect').value=activeProfile||'';$('activeProfileBadge').textContent=activeProfile?prettyProfile(activeProfile):'جدید';$('activeProfileBadge').className='badge '+(activeProfile?'ok':'');$('activeProfileInfo').innerHTML=p?`<b>${esc(activeProfile)}</b> · ${p.saved_products?.length||0} محصول ذخیره‌شده<br><small dir="ltr">${esc(p.url||'')}</small>`:'یک پروفایل را انتخاب کنید؛ انتخاب روی سرور ذخیره می‌شود.'}
