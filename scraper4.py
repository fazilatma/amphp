...
:root{--bg:#08111f;--card:#101c2d;--line:#26364d;--text:#eef5ff;--muted:#9fb0c8;--blue:#38bdf8;--green:#34d399;--red:#fb7185}
*{box-sizing:border-box}body{margin:0;background:linear-gradient(145deg,#07101d,#0d1930);color:var(--text);font-family:Tahoma,Arial,sans-serif;font-size:14px}.wrap{max-width:1180px;margin:auto;padding:24px}h1{font-size:24px;margin:0}.sub{color:var(--muted);margin:8px 0 22px}.tabs{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.tabs button{background:#13233a}.tabs button.on{border-color:var(--blue);color:var(--blue)}.pane{display:none}.pane.on{display:block}.card{background:rgba(16,28,45,.96);border:1px solid var(--line);border-radius:14px;padding:18px;margin-bottom:14px;box-shadow:0 12px 40px #0004}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.grid4{grid-template-columns:repeat(4,minmax(0,1fr))}.wide{grid-column:1/-1}label{display:block;color:var(--muted);font-size:12px;margin-bottom:5px}input,select,textarea,button{font:inherit;border-radius:8px;border:1px solid var(--line);padding:10px;background:#091525;color:var(--text);width:100%}button{width:auto;cursor:pointer;background:#075985;border-color:#0ea5e9;font-weight:bold}button:hover{filter:brightness(1.15)}button.gray{background:#172439;border-color:#34445d}button.green{background:#07624c;border-color:#10b981}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.note{padding:10px;border-radius:8px;background:#13243a;color:#b9cae0;line-height:1.8}.status{white-space:pre-wrap;line-height:1.8;color:#b9cae0}.error{color:var(--red)}.ok{color:var(--green)}table{width:100%;border-collapse:collapse;direction:rtl}th,td{padding:9px;border-bottom:1px solid var(--line);text-align:right;vertical-align:top}th{color:#8fdcff;position:sticky;top:0;background:var(--card)}td img{width:54px;height:54px;object-fit:contain;background:white;border-radius:6px}.tablebox{max-height:560px;overflow:auto}a{color:#7dd3fc}.badge{display:inline-block;padding:4px 8px;border:1px solid #36516f;border-radius:12px;color:#b6d6ef;margin:3px}@media(max-width:720px){.grid,.grid4{grid-template-columns:1fr}.wrap{padding:12px}}
</style></head><body><div class="wrap">
<h1>🕸️ Scraper4 — نسخه PythonAnywhere</h1><div class="sub">استخراج HTML، JSON-LD، دادهٔ hydration، API دیجی‌کالا و رندر اختیاری JavaScript</div>
<div class="tabs"><button class="on" data-tab="scrape">برداشت</button><button data-tab="profiles">پروفایل‌ها</button><button data-tab="settings">تنظیمات</button><button data-tab="woo">ووکامرس</button><button data-tab="deploy">نصب و به‌روزرسانی</button></div>
...
<div class="card"><b>سلکتورهای CSS اختیاری</b><div class="note" style="margin:10px 0">برای React ابتدا حالت «خودکار» را امتحان کنید. سلکتور فقط روی HTML دریافتی یا DOM رندرشده اعمال می‌شود.</div><div class="grid grid4">
...
</div><div class="actions"><button onclick="runScrape()">شروع برداشت</button><button class="gray" onclick="saveProfilePrompt()">ذخیره پروفایل</button><button class="green" onclick="downloadCSV()">CSV نتیجه</button></div></div>
<div id="status" class="card status">آماده</div><div class="card tablebox"><table><thead><tr><th>#</th><th>تصویر</th><th>عنوان</th><th>قیمت</th><th>SKU</th><th>لینک</th></tr></thead><tbody id="rows"></tbody></table></div></section>
...
let products=[],profiles={}; const $=id=>document.getElementById(id); const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
async function runScrape(){$('status').innerHTML='⏳ در حال برداشت… درخواست‌های چندصفحه‌ای ممکن است کمی طول بکشند.';try{let d=await api('/api/scrape',{method:'POST',body:JSON.stringify(config())});products=d.products;renderRows();$('status').innerHTML=`<span class="ok">✓ ${d.total} محصول، ${d.pages} صفحه</span>\nروش: ${esc(d.modes.join(', '))}\n${esc(d.logs.join('\n'))}`;}catch(e){$('status').innerHTML='<span class="error">✗ '+esc(e.message)+'</span>'}}
function renderRows(){$('rows').innerHTML=products.map((p,i)=>`<tr><td>${i+1}</td><td>${p.image?`<img src="${esc(p.image)}" loading="lazy">`:''}</td><td>${esc(p.title)}</td><td dir="ltr">${esc(p.price)}</td><td>${esc(p.sku)}</td><td>${p.link?`<a href="${esc(p.link)}" target="_blank">بازکردن</a>`:''}</td></tr>`).join('')}
