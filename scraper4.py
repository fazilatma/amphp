...
APP_VERSION = "4.6.0"
CHANGELOG = [
    {"version":"4.6.0","date":"2026-09-04","title":"نصب از چند برنچ با انتخاب خودکار جدیدترین نسخه","items":["افزودن چندین برنچ به بخش بروزرسانی با ذخیره فهرست برنچ‌ها","مقایسه خودکار نسخه‌ها در همه برنچ‌ها و نصب برنچ دارای جدیدترین نسخه","گزارش نسخه و وضعیت هر برنچ هنگام بررسی و نمایش برنچ انتخاب‌شده پس از نصب","سازگاری با تنظیمات قبلی تک‌برنچ و جلوگیری از نصب نسخه قدیمی‌تر از نسخه فعال"]},
...
def default_data() -> dict[str, Any]:
    return {
        ...
        "deploy": {
            "repo": "fazilatma/amphp", "branches": ["arena/01a0640f-amphp"], "path": "scraper4.py",
...
def load_data() -> dict[str, Any]:
    with DATA_LOCK:
        try:
            ...
            if isinstance(raw_deploy, dict) and not isinstance(raw_deploy.get("branches"), list) and raw_deploy.get("branch"):
                out["deploy"]["branches"] = [clean_text(raw_deploy["branch"]) or "arena/01a0640f-amphp"]
            return out
...
def deploy_config(data: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    if not isinstance(raw_branches, list) or not raw_branches:
        legacy_branch = clean_text(raw.get("branch"))
        raw_branches = [legacy_branch] if legacy_branch else ["arena/01a0640f-amphp"]
    branches: list[str] = []
    for item in raw_branches:
        branch = clean_text(str(item))
        if branch and branch not in branches:
            branches.append(branch)
    if not branches:
        branches = ["arena/01a0640f-amphp"]
    return {
...
def source_version(content: bytes) -> str:
    """Extract APP_VERSION from Scraper4 source bytes without full validation."""
    try:
        text = content.decode("utf-8")
...
def version_key(value: str) -> tuple[int, ...]:
    """Comparable key for APP_VERSION values such as 4.5.0 or 4.5.0-rc1."""
...
def deploy_candidates(cfg: Optional[dict[str, Any]] = None, include_content: bool = False) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Fetch the source file from every configured branch and return them newest-version first."""
    cfg = cfg or deploy_config()
    ...
    for branch in cfg["branches"]:
        ...
        item["branch"] = branch
        try:
            meta = github_file(item, include_content=include_content)
            version = validate_deploy_source(meta["content"]) if include_content else ""
            candidates.append({
                "branch": branch,
                "version": version,
                "sha": meta["sha"],
...
def deploy_check() -> dict[str, Any]:
    cfg = deploy_config()
    candidates, errors = deploy_candidates(cfg, include_content=True)
    if not candidates:
        detail = "؛ ".join(f"{item['branch']}: {item['error']}" for item in errors) or "هیچ برنچی پیکربندی نشده است"
        raise FetchError("هیچ برنچی برای بررسی در دسترس نیست — " + detail)
    with open(os.path.abspath(__file__), "rb") as fh:
        local = fh.read()
    local_sha = git_blob_sha(local)
    local_version = source_version(local)
    best = candidates[0]
    for index, candidate in enumerate(candidates):
        candidate["newest"] = index == 0
        candidate["local_sha"] = local_sha
        candidate["update_available"] = candidate["sha"] != local_sha or candidate["version"] != local_version
    public_candidates = [{k: v for k, v in item.items() if k != "content"} for item in candidates]
    return {
        "repo": cfg["repo"], "branch": best["branch"], "branches": cfg["branches"], "path": cfg["path"],
        "local_sha": local_sha, "remote_sha": best["sha"],
        "update_available": best["sha"] != local_sha or best["version"] != local_version,
        "remote_size": best["size"], "html_url": best["html_url"],
        "version": best["version"], "current_version": local_version, "app_version": APP_VERSION,
        "candidates": public_candidates, "errors": errors,
    }
...
def deploy_install() -> dict[str, Any]:
    ...
    try:
        ...
        if not candidates:
            detail = "؛ ".join(f"{item['branch']}: {item['error']}" for item in errors) or "هیچ برنچی پیکربندی نشده است"
            raise FetchError("هیچ برنچی برای نصب در دسترس نیست — " + detail)
        best = candidates[0]
        content = best["content"]
        new_version = best["version"]
        versions = {item["branch"]: item["version"] for item in candidates}
        target = os.path.abspath(__file__)
        with open(target, "rb") as fh:
            current = fh.read()
        current_version = source_version(current)
        if version_key(new_version) < version_key(current_version):
            return {"changed": False, "message": f"نسخه نصب‌شده ({current_version}) از جدیدترین نسخه برنچ‌ها تازه‌تر است؛ چیزی تغییر نکرد",
                    "version": current_version, "branch": best["branch"], "branches": versions}
        if git_blob_sha(current) == best["sha"]:
            sdk_installed=ensure_basalam_sdk() if b"basalam-sdk" in content else False
            return {"changed": False, "message": "همین نسخه (جدیدترین برنچ) اکنون نصب است"+("؛ SDK باسلام نیز خودکار نصب شد" if sdk_installed else ""), "version": APP_VERSION,
                    "branch": best["branch"], "branches": versions, "dependencies_repaired": sdk_installed}
        ...
        return {"changed": True, "message": f"نسخه تازه {new_version} از برنچ {best['branch']} اتمیک نصب شد"+(" و SDK باسلام نیز نصب شد" if sdk_installed else ""),
                "version": new_version, "branch": best["branch"], "branches": versions,
                "sha": best["sha"], "backup": os.path.basename(backup), "reload_requested": reloaded, "dependencies_repaired": sdk_installed}
    finally:
        DEPLOY_LOCK.release()
...
def auto_update_worker() -> None:
    ...
    try:
        ...
        candidates, errors = deploy_candidates(cfg, include_content=True)
        if not candidates:
            detail = "؛ ".join(f"{item['branch']}: {item['error']}" for item in errors)
            raise FetchError("هیچ برنچی برای به‌روزرسانی در دسترس نیست" + (f" — {detail}" if detail else ""))
        best = candidates[0]
        target = os.path.abspath(__file__)
        with open(target, "rb") as fh:
            current = fh.read()
        if git_blob_sha(current) == best["sha"]:
            ...
            return
        if version_key(best["version"]) < version_key(source_version(current)):
            return
        if not DEPLOY_LOCK.acquire(blocking=False):
            raise FetchError("یک نصب دیگر هم اکنون در حال اجراست")
        try:
            ...
            AUTO_UPDATE_STATE["error"] = ""
            app.logger.info("Automatically installed Scraper4 %s from %s", best["version"], best["branch"])
...
def settings():
    ...
    if isinstance(body.get("deploy"), dict):
        ...
        raw_branches = body["deploy"].get("branches")
        if not isinstance(raw_branches, list):
            legacy_branch = str(body["deploy"].get("branch", "") or "").strip()
            raw_branches = [legacy_branch] if legacy_branch else None
        if raw_branches is not None:
            ...
            for item in raw_branches:
                ...
                if branch and branch not in branches:
                    branches.append(branch)
            data["deploy"]["branches"] = branches or ["arena/01a0640f-amphp"]
            data["deploy"]["branch"] = data["deploy"]["branches"][0]
...
.branch-list{display:grid;gap:8px;margin-bottom:8px}.branch-row{display:flex;gap:8px;align-items:center}.branch-row input{flex:1;min-width:0}.branch-row button{flex:none;width:46px;min-height:42px;padding:0}.branch-hint{display:block;color:var(--muted);font-size:11px;margin-top:7px;line-height:1.8}.check-branches{display:grid;gap:6px;margin-top:10px}.check-branch{display:flex;flex-wrap:wrap;gap:9px;align-items:center;padding:9px 11px;border:1px solid var(--line);border-radius:11px;background:#081528;font-size:12px}.check-branch.best{border-color:rgba(52,211,153,.4);background:rgba(5,46,34,.55)}.check-branch .best-tag{color:var(--green);font-weight:700}
...
<header class="hero"><div class="hero-main"><div class="logo">🕸️</div><div><div class="eyebrow">مرکز استخراج محصول</div><h1>🛒 اسکرپر <small id="appVersion">v4.6.0</small></h1><div class="sub">استخراج و مدیریت هوشمند محصولات</div></div></div><div class="hero-badge"><span>●</span> آنلاین و آماده</div></header>
...
function addDepBranch(value=''){const box=$('dep_branches');if(!box)return;const row=document.createElement('div');row.className='branch-row';const input=document.createElement('input');input.setAttribute('dir','ltr');input.className='dep-branch-input';input.placeholder='arena/01a0640f-amphp';input.value=value||'';const rm=document.createElement('button');rm.type='button';rm.className='gray';rm.textContent='✕';rm.onclick=()=>row.remove();row.append(input,rm);box.append(row)}
function depBranches(){return [...document.querySelectorAll('#dep_branches .dep-branch-input')].map(x=>x.value.trim()).filter(Boolean)}
function renderDepBranches(list){const box=$('dep_branches');if(!box)return;box.innerHTML='';const items=Array.isArray(list)&&list.length?list:(list?[list]:[]);items.forEach(x=>addDepBranch(x));if(!box.children.length)addDepBranch()}
async function saveDeploy(){let branches=depBranches();if(!branches.length){alert('حداقل یک برنچ وارد کنید');return}let deploy={repo:$('dep_repo').value.trim(),branches,path:$('dep_path').value.trim(),reload_file:$('dep_reload').value.trim(),github_token:$('dep_token').value.trim()};await deployApi('/api/settings',{method:'POST',body:JSON.stringify({deploy})});$('dep_token').value='';$('deployStatus').innerHTML='<span class="ok">تنظیمات نصب ذخیره شد — '+esc(branches.length)+' برنچ ثبت شد.</span>'}
...
async function deployCheck(){try{$('deployStatus').textContent='در حال بررسی همه برنچ‌ها در GitHub…';let d=await deployApi('/api/deploy/check',{method:'POST',body:'{}'});let rows=(d.candidates||[]).map(c=>`<div class="check-branch ${c.newest?'best':''}"><b>${esc(c.branch)}</b><span>v${esc(c.version)}</span><code>${esc(String(c.sha).slice(0,8))}</code>${c.newest?'<b class="best-tag">جدیدترین</b>':''}${c.update_available?'<span class="ok">قابل نصب</span>':'<span>یکسان با نصب فعلی</span>'}</div>`).join('');$('deployStatus').innerHTML=`برنچ انتخاب‌شده: <b>${esc(d.branch)}</b> — v${esc(d.version)}<br>${d.update_available?'<span class="ok">نسخه متفاوت آماده نصب است.</span>':'نسخه محلی و جدیدترین نسخه راه دور یکسان‌اند.'}${rows?'<div class="check-branches">'+rows+'</div>':''}`;}catch(e){$('deployStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
async function deployRun(){if(!confirm('فایل جاری جایگزین و نسخه قبلی در .bak ذخیره شود؟'))return;try{$('deployStatus').textContent='در حال بررسی همه برنچ‌ها، دانلود و نصب جدیدترین نسخه…';let d=await deployApi('/api/deploy/run',{method:'POST',body:'{}'});let summary=(d.branches&&Object.keys(d.branches).length)?'<div class="check-branches">'+Object.entries(d.branches).map(([b,v])=>`<div class="check-branch"><b>${esc(b)}</b><span>v${esc(v)}</span>${b===d.branch?'<b class="best-tag">انتخاب‌شده</b>':''}</div>`).join('')+'</div>':'';$('deployStatus').innerHTML='<span class="ok">'+esc(d.message)+'</span><br>'+(d.reload_requested?'درخواست reload فرستاده شد.':'در صورت تنظیم‌نبودن WSGI، از تب Web دکمه Reload را بزنید.')+summary;}catch(e){$('deployStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
