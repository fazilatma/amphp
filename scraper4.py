...
def fetch_url(url: str, network_cfg: Optional[Dict[str, Any]] = None, timeout: int = 25, headers: Optional[Dict[str, str]] = None) -> str:
    ...
    try:
        ...
        if verify_tls:
            try:
                resp = requests.get(target_url, headers=req_headers, proxies=proxies, timeout=timeout, verify=False)
                resp.raise_for_status()
                resp.encoding = resp.apparent_encoding or "utf-8"
                return resp.text
            except Exception as e2:
                raise FetchError(f"خطا در دریافت نشانی {url}: {str(e2)} (SSL retry)")
...
def _deployer_fetch_version(repo: str, branch: str, token: str = "") -> Optional[Dict[str, Any]]:
    # Try raw first, then GitHub API as fallback (raw fails with SSLZeroReturn in some envs)
    b_enc = urllib.parse.quote(branch, safe="/")
    raw_url = f"https://raw.githubusercontent.com/{repo}/{b_enc}/scraper4.py"
    ...
    for verify in (True, False):
        try:
            r = requests.get(raw_url, headers=headers, timeout=15, verify=verify)
    ...
    if content is None:
        ...
        try:
            for verify in (True, False):
                try:
                    r = requests.get(f"https://api.github.com/repos/{repo}/contents/scraper4.py", params={"ref": branch}, headers=headers, timeout=20, verify=verify)
...
def api_deployer_install():
    ...
    # Now fetch full content for install — raw with SSL fallback, then API
    b_enc = urllib.parse.quote(branch, safe="/")
    ...
    try:
        ...
        if content is None:
            ...
            for verify in (True, False):
                try:
                    r2 = requests.get(f"https://api.github.com/repos/{repo}/contents/scraper4.py", params={"ref": branch}, headers=headers, timeout=20, verify=verify)
                    ...
                    break
        if not content:
            return jsonify(ok=False, error=f"دانلود از {branch} ناموفق بود (HTTP {r_status})"), 500
        # Validate
        try:
            text = content.decode('utf-8')
