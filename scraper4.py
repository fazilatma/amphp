...
def install_basalam_relay_transport() -> None:
    ...
    def request_sync(self,method,path,params=None,data=None,json_data=None,files=None,headers=None,response_model=None,require_auth=True):
        ...
        if not response.ok:
            body=clean_text(response.text)[:700];raise ValueError(f"Basalam relay HTTP {response.status_code}: {body}")
...
def api_basalam_test():
    ...
    try:
        ...
        return jsonify(ok=False,error=hint or text,detail=text[:500],stage="connection" if sdk["installed"] else "install",transport=mode,sdk=sdk),400
