...
def default_data() -> dict[str, Any]:
    return {
        ...
        "ai_candidates": [],
        "ai_master": "",
        "basalam": {"token": "", "refresh_token": "", "vendor_id": 0, "category_id": 0, "preparation_days": 3, "weight": 500, "stock": 10, "update_existing": True, "api_mode": "relay", "relay_url": "https://proxy.fazilat-ma.workers.dev", "worker_key": "", "last_test_at": 0, "last_test_user": ""},
        "bsl_jobs": {},
        "ai_test_jobs": {},
        "dispatch_jobs": {},
    }
...
def woo_request(method: str, endpoint: str, payload: Any = None) -> requests.Response:
    ...
    public_http_url(base);target = base + "/wp-json/wc/v3/" + endpoint.lstrip("/");mode=clean_text(cfg.get("api_mode","relay"));headers={"User-Agent":USER_AGENT,"Accept":"application/json"};auth=None;request_url=target
    if mode=="relay":
        relay=public_http_url(clean_text(cfg.get("relay_url")) or "https://proxy.fazilat-ma.workers.dev")
        # WooCommerce officially accepts consumer credentials in the HTTPS query string. This avoids generic Workers stripping Basic Authorization.
        parsed=urlparse(target);query=parse_qs(parsed.query,keep_blank_values=True);query.update({"consumer_key":[ck],"consumer_secret":[cs]});upstream=urlunparse(parsed._replace(query=urlencode(query,doseq=True)))
        request_url=relay+("&" if "?" in relay else "?")+urlencode({"url":upstream});headers.update({"X-Proxy-UA":USER_AGENT,"X-Proxy-Method":method.upper(),"X-Proxy-Content-Type":"application/json"})
        headers["X-Proxy-Authorization"]="Basic "+base64.b64encode((ck+":"+cs).encode()).decode()
        if cfg.get("worker_key"):headers["X-Proxy-Key"]=str(cfg["worker_key"])
    else:auth=(ck,cs)
    try:response=requests.request(method,request_url,json=payload,auth=auth,timeout=60,headers=headers,allow_redirects=True)
    except requests.RequestException as exc:raise FetchError(f"اتصال ووکامرس از مسیر {mode} ناموفق بود: {exc}") from exc
    if not response.ok:
        body=clean_text(response.text)[:500];hint=" Worker باید متد و بدنه JSON را بدون تغییر عبور دهد." if mode=="relay" and method.upper() in {"POST","PUT","PATCH"} else ""
        raise FetchError(f"WooCommerce {mode} HTTP {response.status_code}: {body}.{hint}")
    return response
...
def api_live_task_retry(task_id: str):
    ...
    cfg=dict(job["config"]);cfg.update({"_start_page":max(1,int(job.get("next_page",1))),"_next_url":job.get("next_url",""),"_resume_products":job.get("products",[])})
    task.update(status="waiting",progress=max(0,int(task.get("progress",0))),step="دوباره در صف FIFO قرار گرفت",cancel_requested=False,updated_at=int(time.time()));task.setdefault("details",[]).append({"at":time.strftime("%H:%M:%S"),"text":"ادامه از آخرین checkpoint درخواست شد"})
...
def scrape_result(report: ScrapeReport) -> dict[str,Any]:
    products=list(report.products.values());data=load_data();previous={product_key(p):p for p in data["last_result"] if isinstance(p,dict)};current={product_key(p):p for p in products}
    ...
    data["last_result"]=products;active=data.get("active_profile","")
    if active in data.get("profiles",{}):data["profiles"][active]["saved_products"]=[dict(x) for x in products]
    save_data(data);return {"products":products,"total":len(products),"pages":report.pages,"modes":sorted(report.modes),"logs":report.logs,"diagnostics":report.diagnostics,"comparison":comparison,"job_id":report.job_id}


def scrape_live_worker(task_id: str, config: dict[str,Any]) -> None:
    live_task_update(task_id,0,"در صف FIFO استخراج","waiting","برای جلوگیری از رقابت شبکه و Playwright، وظایف به ترتیب اجرا می‌شوند")
    with EXTRACT_LOCK:
        if live_task_cancelled(task_id):live_task_update(task_id,0,"پیش از شروع متوقف شد","cancelled","درخواست توقف صف اجرا شد");return
        try:
            live_task_update(task_id,2,"اعتبارسنجی آدرس و تنظیمات","running","نوبت این پروفایل رسید و وظیفه روی سرور آغاز شد");config=dict(config);config["job_id"]=task_id;config["_live_task_id"]=task_id;report=scrape(config);live_task_update(task_id,92,"ذخیره نتایج روی سرور","running",f"{len(report.products)} محصول آماده شد");result=scrape_result(report);live_task_update(task_id,100,"استخراج کامل شد","completed",f"{result['total']} محصول از {result['pages']} صفحه",result=result,done=result['pages'],total=result['pages'],extracted=result['total'],eta_seconds=0)
...
def install_basalam_relay_transport() -> None:
    ...
    def request_sync(self,method,path,params=None,data=None,json_data=None,files=None,headers=None,response_model=None,require_auth=True):
        ...
        relay_url=public_http_url(relay)+("&" if "?" in relay else "?")+urlencode({"url":target});request_headers=self.config.get_headers().copy()
        ...
        request_headers["X-Proxy-UA"]=request_headers.get("User-Agent",USER_AGENT);worker_key=clean_text(getattr(self.config,"scraper4_worker_key",""))
        if worker_key:request_headers["X-Proxy-Key"]=worker_key
        response=requests.request(method,relay_url,headers=request_headers,data=data,json=json_data,files=files,timeout=float(self.config.timeout),allow_redirects=True)
        if not response.ok:
            body=clean_text(response.text)[:700];auth_rejected=response.status_code==401 and ("authorization" in body.lower() or "unauthorized" in body.lower())
            if auth_rejected:
                # The generic v1 Worker intentionally strips Authorization. A 401 is safe to retry because no write occurred.
                try:
                    rewind_upload_files(files);result=original(self,method,path,params,data,json_data,files,headers,response_model,require_auth);setattr(self.config,"scraper4_last_transport","direct-fallback");return result
                except Exception as direct_exc:
                    raise ValueError("Worker باسلام هدر Authorization را به مقصد منتقل نکرد (HTTP 401) و اتصال مستقیم نیز ناموفق بود. Worker عمومی v1.0.2 برای API احراز هویت‌شده مناسب نیست. جزئیات اتصال مستقیم: "+clean_text(direct_exc)[:500]) from direct_exc
            raise ValueError(f"Basalam relay HTTP {response.status_code}: {body}")
...
def basalam_client():
    ...
    sdk_cfg=BasalamConfig(timeout=45,user_agent="Scraper4/"+APP_VERSION);mode=clean_text(cfg.get("api_mode","relay"));relay=clean_text(cfg.get("relay_url"))
    if mode=="relay" and relay:
        install_basalam_relay_transport();sdk_cfg.scraper4_relay_url=relay;sdk_cfg.scraper4_worker_key=clean_text(cfg.get("worker_key"))
...
def api_basalam_test():
    if not deploy_authorized():return deploy_auth_error()
    try:
        was_missing=not basalam_sdk_status()["installed"];user=basalam_client().get_current_user_sync();sdk=basalam_sdk_status();user_name=str(getattr(user,"name",getattr(user,"id","connected")));data=load_data();data["basalam"]["last_test_at"]=int(time.time());data["basalam"]["last_test_user"]=user_name;save_data(data);return jsonify(ok=True,user=user_name,sdk=sdk,installed_now=was_missing and sdk["installed"])
    except Exception as exc:
        text=clean_text(exc);sdk=basalam_sdk_status();hint=""
        mode=clean_text(load_data().get("basalam",{}).get("api_mode","relay"))
        if "403" in text:hint=("Worker واسط درخواست باسلام را با 403 برگرداند؛ مطمئن شوید Worker اجازه عبور Authorization و درخواست‌های POST/PATCH را می‌دهد و کلید Worker درست است." if mode=="relay" else "SDK نصب است، اما API باسلام درخواست را با 403 رد کرد؛ حالت Worker واسط را فعال کنید یا مجوز توکن شخصی و دسترسی غرفه را بررسی کنید.")
        elif "401" in text or "authorization" in text.lower():hint="توکن پاک‌سازی شد اما احراز هویت پذیرفته نشد. Worker عمومی هدر Authorization را حذف می‌کند؛ برنامه اتصال مستقیم را نیز خودکار آزمایش کرده است. Personal Token را بدون عبارت Bearer ذخیره کنید. جزئیات: "+text[:500]
        return jsonify(ok=False,error=hint or text,detail=text[:700],stage="connection" if sdk["installed"] else "install",transport=mode,sdk=sdk),400
...
.task-center{display:grid;gap:10px}.task-card{padding:14px;border:1px solid var(--line);border-radius:16px;background:linear-gradient(145deg,rgba(11,28,50,.94),rgba(14,24,42,.85));position:relative;overflow:hidden}.task-card.running{border-color:#38bdf866}.task-card.completed{border-color:#34d39955}.task-card.failed{border-color:#fb718555}.task-top,.task-metrics,.task-actions{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap}.task-top small{color:var(--muted)}.task-metrics{justify-content:flex-start;margin:9px 0;color:#b9cbe1;font-size:11px}.task-metrics span{padding:4px 8px;border-radius:9px;background:#081528}.task-progress{height:11px;border-radius:99px;background:#06101e;overflow:hidden;border:1px solid #29405d;margin:9px 0}.task-progress i{display:block;height:100%;min-width:2px;border-radius:inherit;background:linear-gradient(90deg,#0ea5e9,#38bdf8,#34d399);background-size:200% 100%;animation:progressGlow 2s linear infinite;transition:width .45s ease}.task-card.failed .task-progress i{background:#fb7185}.task-step{font-weight:700;color:#d9f2ff;margin-top:8px}.task-timeline{max-height:155px;overflow:auto;margin-top:9px;padding-right:8px;border-right:2px solid #244665}.task-event{font-size:11px;color:#aebfd5;padding:4px 8px;overflow-wrap:anywhere}.task-event b{color:#70d7ff}.task-actions{justify-content:flex-end;margin-top:9px}.task-actions button{font-size:11px;min-height:34px;padding:6px 10px}.changelog{display:grid;gap:10px}.release{padding:14px;border:1px solid var(--line);border-radius:16px;background:#09172a}.release.current{border-color:#34d39966;background:linear-gradient(140deg,#0b2929,#10223b)}.release-head{display:flex;justify-content:space-between;gap:8px}.release ul{margin:9px 0 0;padding-right:20px;color:#b8c9dd}.dispatch-hero{border-color:#38bdf855;background:linear-gradient(135deg,rgba(2,132,199,.16),rgba(16,185,129,.10))}@keyframes progressGlow{to{background-position:-200% 0}}
...
let taskAutoRefresh=true,taskRefreshTimer=0;
function taskCard(t){const p=Math.max(0,Math.min(100,Number(t.progress||0))),elapsed=t.elapsed_seconds??Math.max(0,Math.floor(Date.now()/1000-Number(t.created_at||Date.now()/1000))),active=['waiting','running'].includes(t.status);let destination=Object.entries(t.destinations||{}).map(([k,v])=>(k==='woocommerce'?'ووکامرس':'باسلام')+`: ${v.sent||0} موفق / ${v.failed||0} خطا`).join(' · ');return `<article class="task-card ${esc(t.status)}"><div class="task-top"><div><b>${esc(t.title||t.kind)}</b><br><small>${esc(t.profile||t.kind||'')} · ${new Date((t.created_at||0)*1000).toLocaleString('fa-IR')}</small></div><span class="badge ${t.status==='completed'?'ok':t.status==='failed'?'error':''}">${esc(taskStatusLabel(t.status))}</span></div><div class="task-progress"><i style="width:${p}%"></i></div><div class="task-top"><span class="task-step">${esc(t.step||'')}</span><b>${p}٪</b></div><div class="task-metrics"><span>انجام ${t.done??'—'} / ${t.total??'—'}</span><span class="ok">موفق ${t.sent??'—'}</span><span class="error">خطا ${t.failed??'—'}</span><span>سپری‌شده ${shortDuration(elapsed)}</span>${active&&t.eta_seconds!=null?`<span>زمان باقی‌مانده ≈ ${shortDuration(t.eta_seconds)}</span>`:''}</div>${destination?`<small>${esc(destination)}</small>`:''}<div class="task-timeline">${(t.details||[]).map(x=>`<div class="task-event"><b>${esc(x.at)}</b> · ${esc(x.text)}</div>`).join('')||'<div class="task-event">هنوز رویدادی ثبت نشده است.</div>'}</div><div class="task-actions">${active?`<button class="gray" onclick="cancelTask('${esc(t.id)}')">توقف امن</button>`:(t.kind==='scrape'&&['failed','interrupted','cancelled'].includes(t.status)?`<button onclick="retryTask('${esc(t.id)}')">↻ ادامه از checkpoint</button>`:'')}<button class="gray" onclick="deleteTask('${esc(t.id)}')">حذف گزارش</button></div></article>`}
async function loadTaskManager(){clearTimeout(taskRefreshTimer);try{let d=await deployApi('/api/tasks');$('taskManagerBadge').textContent=(d.summary.running||0)+' فعال';$('taskManagerStats').innerHTML=`<div class="space-card"><b>${d.tasks.length}</b><span>کل گزارش‌ها</span></div><div class="space-card"><b>${d.summary.running}</b><span>در حال اجرا</span></div><div class="space-card"><b>${d.summary.completed}</b><span>کامل</span></div><div class="space-card"><b>${d.summary.failed}</b><span>ناموفق</span></div>`;$('taskManagerList').innerHTML=(d.tasks||[]).map(taskCard).join('')||'<div class="note">هنوز وظیفه‌ای ثبت نشده است.</div>';$('taskAutoBtn').textContent='پایش خودکار: '+(taskAutoRefresh?'روشن':'خاموش')}catch(e){$('taskManagerList').innerHTML='<span class="error">'+esc(e.message)+'</span>'}if(taskAutoRefresh&&$('jobs')?.classList.contains('admin-on'))taskRefreshTimer=setTimeout(loadTaskManager,1800)}
...
async function init(){let d=await api('/api/config');currentBuild=d.build||'';$('appVersion').textContent='v'+(d.version||'3.3.0');profiles=d.profiles||{};$('timeout').value=d.network.timeout;$('gap_ms').value=d.network.gap_ms;$('proxy').value=d.network.proxy||'';$('proxy_mode').value=d.network.proxy_mode||'auto';$('worker_key').value=d.network.worker_key||'';$('verify_tls').checked=d.network.verify_tls!==false;$('woo_url').value=d.woocommerce.url||'';$('woo_ck').value=d.woocommerce.consumer_key||'';$('woo_cs').value=d.woocommerce.consumer_secret||'';$('woo_api_mode').value=d.woocommerce.api_mode||'relay';$('woo_relay_url').value=d.woocommerce.relay_url||'https://proxy.fazilat-ma.workers.dev';$('woo_worker_key').value=d.woocommerce.worker_key||'';$('dep_repo').value=d.deploy.repo||'';$('dep_branch').value=d.deploy.branch||'';$('dep_path').value=d.deploy.path||'';$('dep_reload').value=d.deploy.reload_file||'';$('dep_token').placeholder=d.deploy.has_github_token?'توکن تنظیم شده است؛ خالی = نگه‌داشتن':'GitHub token اختیاری';activeProfile=d.active_profile||'';renderProfiles();if(activeProfile&&profiles[activeProfile])loadProfile(activeProfile,false,false);else updateActiveProfileUI();loadJobs();loadWooJobs();let saved=localStorage.getItem('scraperActiveTab');openTab(['scrape','profileSettings','selectors','results','woo','imports'].includes(saved)?saved:'scrape')}
async function loadBasalam(){try{let d=await deployApi('/api/basalam/config'),b=d.basalam,s=d.sdk||{};$('bsl_vendor').value=b.vendor_id||0;$('bsl_category').value=b.category_id||0;$('bsl_token').value=b.token||'';$('bsl_refresh').value=b.refresh_token||'';$('bsl_api_mode').value=b.api_mode||'relay';$('bsl_relay_url').value=b.relay_url||'https://proxy.fazilat-ma.workers.dev';$('bsl_worker_key').value=b.worker_key||'';$('bsl_days').value=b.preparation_days||3;$('bsl_weight').value=b.weight||500;$('bsl_stock').value=b.stock||10;$('bsl_update').checked=b.update_existing!==false;$('bslSdkBadge').textContent=s.installed?'SDK '+s.version:'نصب نشده';$('bslSdkBadge').className='badge '+(s.installed?'ok':'error');if(b.last_test_at)$('bslAdminStatus').innerHTML='<span class="ok">آخرین اتصال موفق: '+esc(b.last_test_user||'باسلام')+' · '+new Date(b.last_test_at*1000).toLocaleString('fa-IR')+'</span>'}catch(e){$('bslAdminStatus').textContent=e.message}}
async function saveBasalam(){try{let basalam={vendor_id:+$('bsl_vendor').value,category_id:+$('bsl_category').value,token:$('bsl_token').value.trim(),refresh_token:$('bsl_refresh').value.trim(),api_mode:$('bsl_api_mode').value,relay_url:$('bsl_relay_url').value.trim(),worker_key:$('bsl_worker_key').value.trim(),preparation_days:+$('bsl_days').value,weight:+$('bsl_weight').value,stock:+$('bsl_stock').value,update_existing:$('bsl_update').checked};await deployApi('/api/basalam/settings',{method:'POST',body:JSON.stringify({basalam})});$('bslAdminStatus').innerHTML='<span class="ok">اتصال ذخیره شد.</span>'}catch(e){$('bslAdminStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
...
let activeAITestJob='',aiTestJobData=null,aiAutoRunning=false,aiResultPage=1;
...
function renderAITestModal(){let job=aiTestJobData;if(!job)return;let providerSel=$('aiResultProvider'),current=providerSel.value||'all',providers=[...new Set((job.rows||[]).map(r=>r.provider))];providerSel.innerHTML='<option value="all">همه ارائه‌دهندگان</option>'+providers.map(id=>`<option value="${esc(id)}">${esc((job.rows.find(r=>r.provider===id)||{}).provider_name||id)}</option>`).join('');providerSel.value=providers.includes(current)?current:'all';let q=($('aiResultSearch').value||'').toLowerCase(),filter=$('aiResultFilter').value,cap=$('aiResultCapability').value,sort=$('aiResultSort').value;let rows=(job.rows||[]).filter(r=>{let st=testState(r),hay=[r.provider_name,r.model_name,r.model,r.reply,r.category,r.error].join(' ').toLowerCase(),capOk=cap==='all'||cap==='free'&&r.free||cap==='reasoning'&&r.reasoning||cap==='vision'&&r.vision||cap==='tools'&&r.tool_calling;return (!q||hay.includes(q))&&(providerSel.value==='all'||r.provider===providerSel.value)&&(filter==='all'||st===filter)&&capOk});rows.sort((a,b)=>sort==='fast'?(a.latency_ms||1e12)-(b.latency_ms||1e12):sort==='provider'?String(a.provider_name).localeCompare(String(b.provider_name),'fa'):sort==='model'?String(a.model_name).localeCompare(String(b.model_name),'fa'):sort==='status'?testState(a).localeCompare(testState(b)):(b.score||0)-(a.score||0));let size=+$('aiResultPageSize').value||50,pages=Math.max(1,Math.ceil(rows.length/size));aiResultPage=Math.min(aiResultPage,pages);let shown=rows.slice((aiResultPage-1)*size,aiResultPage*size);$('aiResultPageInfo').textContent=`صفحه ${aiResultPage} از ${pages} · ${rows.length} نتیجه`;$('aiModalSubtitle').textContent=`${job.cursor} از ${job.total} مدل · پیام مشتری: ${job.options?.reply_message||''} · محصول: ${job.options?.category_title||''}`;$('aiModalStats').innerHTML=`<div class="space-card"><b>${job.ok_count}</b><span>کامل</span></div><div class="space-card"><b>${job.reply_ok}</b><span>پاسخ مشتری</span></div><div class="space-card"><b>${job.category_ok}</b><span>دسته‌بندی</span></div><div class="space-card"><b>${job.waiting}</b><span>باقی‌مانده</span></div><div class="space-card"><b>${esc(job.recommended?.model_name||'—')}</b><span>پیشنهاد برتر · ${job.recommended?.score||0}</span></div>`;$('aiModalRows').innerHTML=shown.map((r,i)=>`<tr><td>${(aiResultPage-1)*size+i+1}</td><td><b>${esc(r.provider_name)}</b><br>${esc(r.model_name)}<br><code>${esc(r.model)}</code></td><td>${r.free?'<i class="cap-dot">رایگان</i>':''}${r.reasoning?'<i class="cap-dot">استدلال</i>':''}${r.vision?'<i class="cap-dot">تصویر</i>':''}${r.tool_calling?'<i class="cap-dot">ابزار</i>':''}<br><b>${r.score??0} امتیاز</b></td><td class="answer-cell">${r.reply_ok?'<span class="ok">✓</span> ':'<span class="error">✕</span> '}${esc(r.reply||r.reply_error||'در انتظار')}</td><td class="answer-cell">${r.category_ok?'<span class="ok">✓</span> ':'<span class="error">✕</span> '}${esc(r.category||r.category_error||'در انتظار')}</td><td>${r.reply_ms||0} + ${r.category_ms||0}<br><b>${r.latency_ms||0} ms</b></td><td><span class="badge ${testState(r)==='ok'?'ok':testState(r)==='waiting'?'':'error'}">${testState(r)==='ok'?'سالم':testState(r)==='partial'?'ناقص':testState(r)==='failed'?'خطا':'انتظار'}</span></td></tr>`).join('')||'<tr><td colspan="7" class="empty">نتیجه‌ای مطابق فیلتر نیست.</td></tr>'}
...
async function saveSettings(woo=false){let body={network:{timeout:+$('timeout').value,gap_ms:+$('gap_ms').value,proxy:$('proxy').value.trim(),proxy_mode:$('proxy_mode').value,worker_key:$('worker_key').value.trim(),verify_tls:$('verify_tls').checked}};if(woo)body.woocommerce={url:$('woo_url').value.trim(),consumer_key:$('woo_ck').value.trim(),consumer_secret:$('woo_cs').value.trim(),api_mode:$('woo_api_mode').value,relay_url:$('woo_relay_url').value.trim(),worker_key:$('woo_worker_key').value.trim()};await deployApi('/api/settings',{method:'POST',body:JSON.stringify(body)});alert('ذخیره شد')}
async function wooTest(){const target=$('wooConnectionStatus')||$('wooStatus');try{target.textContent='در حال تست اتصال ووکامرس از مسیر '+($('woo_api_mode').value==='relay'?'Cloudflare Worker':'مستقیم')+'…';let d=await deployApi('/api/woo/test',{method:'POST',body:'{}'});target.innerHTML='<span class="ok">اتصال ووکامرس از مسیر '+($('woo_api_mode').value==='relay'?'Cloudflare Worker':'مستقیم')+' موفق است.</span>'}catch(e){target.innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
