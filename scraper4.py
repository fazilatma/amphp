...
#
# v5.2.0 — replaced with the user's catalogue
#   ai-providers-2026-08-23-160523-gemini-llm7-opencode-ollama-cloud-added.json
#   (replace semantics: فقط فایل کاربر). API keys stripped for security
#   (original file contained sk-or-v1-...). The pasted file was truncated in
#   chat, so only the readable part (ollama + openrouter with 12 models) is
#   embedded here; the user should re-paste the full file in chunks or via
#   a gist to get the complete model list.
DEFAULT_AI_PROVIDERS: Dict[str, Any] = {
    ...
    "openrouter": {
        ...
        "models": [
            ...
            {"id": "liquid/lfm-2.5-2.6b:free", "name": "LiquidAI: LFM2.5-2.6B (free)", "free": True},
...
# Separate deployer address — independent page for multi-branch work
@app.get("/deployer")
@app.get("/deployer/")
@app.get("/deploy")
@app.get("/deploy/")
def route_deployer():
    ...
    if auth_err:
        return auth_err
    return Response(DEPLOYER_PAGE_HTML, mimetype="text/html; charset=utf-8")

@app.get("/deployer.py")
def route_deployer_py():
    """Serve the standalone deployer.py source for direct download at /deployer.py."""
    auth_err = check_auth()
    if auth_err:
        return auth_err
    # Prefer the sibling deployer.py next to this file
    here = os.path.dirname(os.path.abspath(__file__)) or "."
    path = os.path.join(here, "deployer.py")
    if os.path.isfile(path):
        return send_file(path, mimetype="text/x-python", as_attachment=False, download_name="deployer.py")
    return Response("# deployer.py not found on server\n", mimetype="text/plain", status=404)
...
# ==================== SYSTEM INFO (PHP version parity) ====================
...
def api_system_info():
    ...
    try:
        import flask
        info["flask_version"] = getattr(flask, "__version__", "") or ""
    except:
        pass
    # Try PHP version if php binary exists
    try:
        out = subprocess.run(["php", "-v"], capture_output=True, text=True, timeout=5)
        if out.returncode == 0:
            first = (out.stdout or "").splitlines()[0] if out.stdout else ""
            info["php_version"] = first[:120]
            # Try to get SAPI
            out2 = subprocess.run(["php", "-r", "echo PHP_SAPI;"], capture_output=True, text=True, timeout=5)
            if out2.returncode == 0:
                info["php_sapi"] = (out2.stdout or "").strip()[:50]
    except:
        info["php_version"] = "نصب نیست یا در دسترس نیست"
    # Disk usage (home)
    try:
    ...
    # Env hints
    info["env"] = {
        "SCRAPER_DATA_FILE": get_data_filepath(),
        "USER": os.environ.get("USER", ""),
        "PWD": os.getcwd(),
        "PLAYWRIGHT_BROWSERS_PATH": os.environ.get("PLAYWRIGHT_BROWSERS_PATH", "")
    }
    # Extra: list files in app dir
    try:
        app_dir = os.path.dirname(os.path.abspath(__file__)) or "."
        files = []
        for fn in os.listdir(app_dir):
            if fn.startswith("scraper4.py"):
                fp = os.path.join(app_dir, fn)
                try:
                    sz = os.path.getsize(fp)
                    files.append({"name": fn, "size": sz, "size_kb": round(sz/1024, 1)})
                except:
                    pass
        info["app_files"] = sorted(files, key=lambda x: x["name"])[:20]
    except:
        info["app_files"] = []
    return jsonify(ok=True, info=info)


@app.get("/api/system/php-version")
def api_system_php_version():
    auth_err = check_auth()
    if auth_err:
        return auth_err
    # Simple endpoint for PHP version tab
    php_ver = "نصب نیست"
    sapi = ""
    try:
        out = subprocess.run(["php", "-r", "echo PHP_VERSION . '|' . PHP_SAPI;"], capture_output=True, text=True, timeout=5)
        if out.returncode == 0 and out.stdout:
            parts = out.stdout.strip().split("|")
            php_ver = parts[0] if len(parts) > 0 else out.stdout.strip()
            sapi = parts[1] if len(parts) > 1 else ""
    except Exception as e:
        php_ver = f"خطا: {e}"
    return jsonify(ok=True, php_version=php_ver, php_sapi=sapi, app_version=APP_VERSION,
                   python_version=__import__("platform").python_version())


# ==================== DEPLOYER AUTO (search all branches) ====================
def _deployer_parse_version(v: str) -> Tuple[int, ...]:
...
def _deployer_list_branches(repo: str, token: str = "") -> List[Dict[str, Any]]:
    headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "scraper4-deployer"}
    if token:
...
def _deployer_fetch_version(repo: str, branch: str, token: str = "") -> Optional[Dict[str, Any]]:
    # Fetch raw file
    try:
        # branch may contain slashes, keep them
        b_enc = urllib.parse.quote(branch, safe="/")
        url = f"https://raw.githubusercontent.com/{repo}/{b_enc}/scraper4.py"
        headers = {"User-Agent": "scraper4-deployer"}
        if token:
            headers["Authorization"] = f"token {token}"
        r = requests.get(url, headers=headers, timeout=15)
        ...
        return {
            "branch": branch,
            "version": ver,
            "version_tuple": _deployer_parse_version(ver),
            "size": len(r.content),
            "sha": ""
        }
    except Exception:
...
@app.post("/api/deployer/scan")
def api_deployer_scan():
    auth_err = check_auth()
    if auth_err:
        return auth_err
    body = request.get_json(silent=True) or {}
    repo = (body.get("repo") or DEFAULT_GITHUB_REPO).strip() or DEFAULT_GITHUB_REPO
    filter_str = (body.get("filter") or "").strip().lower()
    token = (body.get("token") or load_data().get("deploy", {}).get("github_token") or "").strip()
    branches_data = _deployer_list_branches(repo, token)
    results = []
    ...
    local_tuple = _deployer_parse_version(local_ver)
    return jsonify(ok=True, repo=repo, local_version=local_ver, local_tuple=local_tuple,
                   count=len(results), branches=results[:50],
                   newest=results[0] if results else None)

@app.post("/api/deployer/install")
def api_deployer_install():
    auth_err = check_auth()
    if auth_err:
        return auth_err
    body = request.get_json(silent=True) or {}
    ...
    # If no branch specified, find newest (with optional filter like zip/arena/deploy)
    if not branch:
        filter_raw = (body.get("filter") or "").strip()
        keywords = [k.strip().lower() for k in filter_raw.split(",") if k.strip()] if filter_raw else []
        branches_data = _deployer_list_branches(repo, token)
        if keywords:
            filtered = [b for b in branches_data if any(kw in (b.get("name") or "").lower() for kw in keywords)]
            branches_data = filtered or branches_data  # fallback to all if filter yields nothing
        results = []
        for b in branches_data:
            name = b.get("name") or ""
            if not name:
            ...
            info = _deployer_fetch_version(repo, name, token)
            if info:
                info["sha"] = (b.get("commit", {}) or {}).get("sha", "")[:7]
                results.append(info)
        if not results:
            return jsonify(ok=False, error="هیچ برنچی با scraper4.py معتبر پیدا نشد"), 404
        results.sort(key=lambda x: (x["version_tuple"], x["branch"]), reverse=True)
        newest = results[0]
        branch = newest["branch"]
    # Now fetch full content for install
    b_enc = urllib.parse.quote(branch, safe="/")
    url = f"https://raw.githubusercontent.com/{repo}/{b_enc}/scraper4.py"
    try:
        headers = {"User-Agent": "scraper4-deployer"}
        ...
        if r.status_code != 200 or not r.content:
            return jsonify(ok=False, error=f"دانلود از {branch} ناموفق بود (HTTP {r.status_code})"), 500
        content = r.content
        # Validate
        try:
            text = content.decode('utf-8')
            if "APP_VERSION" not in text or "Flask(" not in text:
                return jsonify(ok=False, error="فایل دانلودشده معتبر نیست"), 500
            import ast
            ast.parse(text)
        except Exception as e:
            return jsonify(ok=False, error=f"اعتبارسنجی شکست خورد: {e}"), 500
        # Check version vs local
        m = re.search(r'APP_VERSION\s*=\s*["\']([^"\']+)["\']', text)
        remote_ver = m.group(1) if m else "unknown"
        local_tuple = _deployer_parse_version(APP_VERSION)
        remote_tuple = _deployer_parse_version(remote_ver)
        if not force and remote_tuple <= local_tuple:
            return jsonify(ok=True, changed=False, message=f"نسخه فعلی {APP_VERSION} جدیدتر یا مساوی {remote_ver} است — نصب نشد (force برای اجبار)",
                           local_version=APP_VERSION, remote_version=remote_ver, branch=branch)
        # Install
        target_path = os.path.abspath(__file__)
        import shutil
        ...
        data = load_data()
        data.setdefault("deploy", {})["branch"] = branch
        data["deploy"]["repo"] = repo
        save_data(data)
        return jsonify(ok=True, changed=True, message=f"✅ {branch} نسخه {remote_ver} نصب شد",
                       local_version=APP_VERSION, remote_version=remote_ver, branch=branch, backup=os.path.basename(bak) if bak else "")
    except Exception as e:
...
# Standalone Deployer page — separate address (/deployer) for multi-branch work
DEPLOYER_PAGE_HTML = r"""<!doctype html>
<html lang="fa" dir="rtl">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="theme-color" content="#07111f">
<title>📦 دیپلوی مستقل — انتخاب برنچ با نسخه</title>
<link href="https://cdn.jsdelivr.net/gh/rastikerdar/vazirmatn@v33.003/Vazirmatn-font-face.css" rel="stylesheet"/>
<style>
:root{--bg:#07111f;--card:#0e1d35;--border:rgba(148,177,216,.16);--text:#f1f5f9;--dim:#94a3b8;--pri:#38bdf8;--acc:#818cf8;--ok:#34d399;--warn:#fbbf24;--danger:#fb7185;--r:14px}
*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 85% -10%,rgba(37,99,235,.28),transparent 40%),linear-gradient(160deg,#07111f,#0d1b33);color:var(--text);font-family:'Vazirmatn',Tahoma,sans-serif;padding:0;min-height:100vh}
...
#log{white-space:pre-wrap;font-family:monospace;font-size:11px;max-height:240px;overflow:auto;background:#020617;padding:10px;border-radius:10px;border:1px solid var(--border)}
.kbd{font-family:monospace;background:rgba(255,255,255,.08);padding:1px 6px;border-radius:6px;font-size:11px}
</style>
</head>
<body>
<div class="header">
  <span style="font-weight:900">📦 دیپلوی مستقل</span>
  <span class="badge" id="localVer">v—</span>
  <span class="badge" id="branchCount">— برنچ</span>
  <div style="flex:1"></div>
  <a class="btn btn-sec" href="/" target="_blank">🏠 اپ اصلی ↗</a>
  <a class="btn btn-sec" href="/health" target="_blank">health</a>
</div>
<div class="wrap">
  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
      <h3>آدرس جداگانه برای کار هم‌زمان با چند برنچ</h3>
      <span class="badge">هر تب یک فیلتر جدا → چند برنچ هم‌زمان</span>
    </div>
    <p style="color:var(--dim);font-size:12px;line-height:1.8;margin:6px 0 10px">
      این صفحه <b>مستقل از تب اپ اصلی</b> است. می‌توانید آن را در چند تب با فیلترهای مختلف باز کنید
      (<span class="kbd">?filter=arena</span> ، <span class="kbd">?filter=zip</span> …) و هر برنچ را با <b>شماره نسخه</b> ببینید و نصب کنید.
      پارامترهای URL ذخیره می‌شوند؛ کپی لینک همین صفحه، همان فیلتر/برنچ را به همکار می‌دهد.
    </p>
    <div class="grid">
      <label>مخزن<input id="repo" value="fazilatma/amphp" dir="ltr" placeholder="owner/repo"></label>
      <label>فیلتر برنچ<input id="filter" value="zip,arena,deploy" placeholder="خالی=همه، مثلا arena,zip"></label>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
      ...
      <button class="btn btn-sec" onclick="copyLink()">🔗 کپی لینک این فیلتر</button>
      <button class="btn btn-sec" onclick="openNewTab()">↗ تب جدید</button>
    </div>
  ...
  <div class="card">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
      ...
      <a id="rawLink" class="btn btn-sec" href="#" target="_blank" style="display:none">📄 نمایش فایل خام</a>
      <a id="ghLink" class="btn btn-sec" href="#" target="_blank" style="display:none">🔗 گیت‌هاب</a>
    </div>
    <div style="margin-top:8px;color:var(--dim);font-size:11px">نکته: برای کار هم‌زمان، هر تب را با فیلتر متفاوت باز کنید: <span class="kbd">/deployer?filter=arena</span> | <span class="kbd">/deployer?filter=zip</span></div>
  </div>

  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
      <h3>📋 لیست برنچ‌ها (با نسخه)</h3>
...
function authHeader(){try{return localStorage.getItem('scraper_auth')||''}catch(e){return ''}}
async function apiFetch(url, opts={}){
  opts.headers=Object.assign({'Content-Type':'application/json'}, opts.headers||{});
  const tok=authHeader(); if(tok) opts.headers['X-Scraper-Auth']=tok;
  let r=await fetch(url, opts);
  if(r.status===401){
    const pw=prompt('رمز اپ (SCRAPER_PASSWORD) را وارد کنید:');
    if(pw){ try{localStorage.setItem('scraper_auth', pw)}catch(e){}; opts.headers['X-Scraper-Auth']=pw; r=await fetch(url, opts); }
  }
  return r;
}
function syncUrl(){
  const repo=$('repo').value.trim(), filter=$('filter').value.trim(), br=$('branchSelect').value||'';
  const u=new URL(location.href); u.searchParams.set('repo', repo); u.searchParams.set('filter', filter); if(br) u.searchParams.set('branch', br); else u.searchParams.delete('branch');
...
function onBranchChange(){
  ...
  const repo=$('repo').value.trim()||'fazilatma/amphp';
  raw.href=`https://raw.githubusercontent.com/${repo}/${encodeURIComponent(br).replace(/%2F/g,'/')}/scraper4.py`;
  raw.style.display=''; raw.textContent='📄 scraper4.py @ '+br;
...
async function scan(){
  const repo=$('repo').value.trim()||'fazilatma/amphp', filter=$('filter').value.trim();
  ...
  syncUrl();
  try{
    const res=await apiFetch('/api/deployer/scan',{method:'POST', body:JSON.stringify({repo, filter})});
    ...
    const cmp = newest ? (JSON.stringify(newest.version_tuple||[])>JSON.stringify(j.local_tuple||[]) ? 'جدیدتر موجود است ⬆️' : 'به‌روز هستید ✅') : '';
    const newestInfo = newest ? `<b>v${esc(newest.version)} (${esc(newest.branch)})</b> — ${cmp}` : 'یافت نشد';
    status.innerHTML=`محلی: <b>v${esc(local)}</b> | جدیدترین: ${newestInfo} | ${toFa(j.count)} برنچ دارای فایل`;
    logLine(`اسکن: ${j.count} برنچ، جدیدترین ${newest?.branch||'—'} v${newest?.version||'—'}`);
  ...
  table.innerHTML=rows.map(r=>{
    ...
    return `<div class="row ${isN?'is-newest':''}">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        ...
        <a class="btn btn-sec" href="https://raw.githubusercontent.com/${esc($('repo').value.trim()||'fazilatma/amphp')}/${esc(r.branch).replace(/'/g,'&#39;').replace(/%2F/g,'/')}/scraper4.py" target="_blank">📄 خام</a>
      </div>
    </div>`;
  }).join('');
}
async function doInstall(branch, force){
  const repo=$('repo').value.trim()||'fazilatma/amphp', status=$('status');
  status.textContent=`⏳ نصب ${branch}...`; logLine(`نصب ${branch} force=${!!force}`);
  try{
    const res=await apiFetch('/api/deployer/install',{method:'POST', body:JSON.stringify({repo, branch, force:!!force})});
    const j=await res.json();
    if(j.ok && j.changed){ status.innerHTML=`✅ ${esc(j.message)}<br>محلی v${esc(j.local_version)} → v${esc(j.remote_version)} (${esc(j.branch)})`; logLine(j.message); setTimeout(()=>location.reload(),1800); }
...
async function installBranch(b, force){ if(!b) return; if(!confirm(`برنچ «${b}»${force?' (اجباری)':''} نصب شود؟`)) return; await doInstall(b,force); }
async function installSelected(force){
  const sel=$('branchSelect'); if(!sel.value){alert('ابتدا برنچ انتخاب کنید'); return;}
  if(!confirm(`برنچ «${sel.value}»${force?' (اجباری)':''} نصب شود؟`)) return; await doInstall(sel.value, force);
}
async function autoInstall(){
  const repo=$('repo').value.trim()||'fazilatma/amphp', filter=$('filter').value.trim(), status=$('status');
  if(!confirm('نصب خودکار جدیدترین نسخه (با فیلتر فعلی)؟')) return;
  status.textContent='⏳ نصب خودکار...';
  try{
    const res=await apiFetch('/api/deployer/install',{method:'POST', body:JSON.stringify({repo, filter})});
    const j=await res.json();
    if(j.ok&&j.changed){ status.innerHTML=`✅ ${esc(j.message)}`; logLine(j.message); setTimeout(()=>location.reload(),1800); }
...
(function(){
  ...
  // auto scan on load so each tab starts ready
  setTimeout(scan, 300);
})();
</script>
</body>
...
<aside class="drawer-panel" id="drawerPanel">
  ...
  <div class="drawer-body">
    ...
    <div class="drawer-menu-item" onclick="drawerNavigate('settings', 'deploy')">
      ...
      <span class="chip chip-accent">v5.3.2</span>
    </div>

    <div class="drawer-menu-item" onclick="window.open('/deployer','_blank'); toggleDrawer();">
      <div><span class="d-icon">📦</span><span>دیپلوی مستقل — آدرس جداگانه + نسخه</span></div>
      <span class="chip chip-accent">/deployer ↗</span>
    </div>
...
<div class="app-shell">
  ...
  <main id="tab-start" class="view-panel active">
    <!-- Profile Management (PHP parity) -->
    <div class="glass-card">
      <div class="card-header">
        <div class="card-title">
          <span class="card-title-icon">📚</span>
          <span>پروفایل‌های ذخیره شده (مثل PHP)</span>
        </div>
        <span class="chip chip-primary" id="profileCountChip">۰ پروفایل</span>
      </div>
      <div class="form-grid" style="grid-template-columns: 1.5fr 1fr auto auto;">
        <select class="form-control" id="profileSelect" onchange="onProfileChange(this.value)">
          <option value="">-- انتخاب سایت ذخیره شده --</option>
        </select>
        <input type="text" class="form-control" id="profileName" placeholder="🏷️ نام دلخواه پروفایل" oninput="scheduleAutoSave()">
        <button class="btn btn-cyan btn-sm" onclick="saveCurrentProfile()">💾 ذخیره</button>
        <button class="btn btn-danger btn-sm" onclick="deleteActiveProfile()">🗑️ حذف</button>
      </div>

      <div style="margin-top:12px;padding:10px;background:rgba(15,23,42,0.8);border:1px solid var(--border);border-radius:var(--radius-md)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer">
            <input type="checkbox" id="profileSyncEn" onchange="scheduleAutoSave();updateSyncStatusText()">
            <b style="color:var(--primary);font-size:calc(12px * var(--font-scale,1))">🔄 همگام‌سازی دوره‌ای این پروفایل</b>
          </label>
        </div>
        <div class="form-grid" style="grid-template-columns: 1fr 1fr;">
          <div class="form-group">
            <label class="form-label" style="font-size:calc(11px * var(--font-scale,1))">دوره:</label>
            <select class="form-control" id="profileSyncInterval" onchange="scheduleAutoSave();updateSyncStatusText()">
          ...
          <div class="form-group">
            <label class="form-label" style="font-size:calc(11px * var(--font-scale,1))">هدف ارسال:</label>
            <select class="form-control" id="profileSyncTarget" onchange="scheduleAutoSave();updateSyncStatusText()">
              <option value="woo">ووکامرس</option>
              <option value="bsl">باسلام</option>
              <option value="both">هر دو</option>
              <option value="none">هیچ‌کدام (فقط استخراج)</option>
            </select>
          </div>
        </div>
        <div style="margin-top:8px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:calc(11px * var(--font-scale,1))">
            <input type="checkbox" id="profileSyncNoExtract" onchange="scheduleAutoSave();updateSyncStatusText();syncToggleNoExtractBox()">
            <span style="color:#fbbf24">🚫 بدون استخراج — فقط آپدیت قیمت/موجودی (برای CSV)</span>
          </label>
          <div id="syncNoExtractBox" style="display:none;margin-top:6px;padding:8px;background:rgba(26,46,5,0.5);border:1px solid #65a30d;border-radius:6px">
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:calc(11px * var(--font-scale,1))">
              <input type="checkbox" id="profileSyncNoExtractRefresh" checked onchange="scheduleAutoSave()">
              <span style="color:#bef264">🔄 تازه‌سازی قیمت از صفحه محصول</span>
            </label>
            <div id="syncNoExtractStat" style="font-size:calc(10px * var(--font-scale,1));color:var(--primary);margin-top:4px"></div>
          </div>
        </div>
        <div style="margin-top:8px;display:flex;flex-direction:column;gap:4px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:calc(11px * var(--font-scale,1))">
            <input type="checkbox" id="profileSyncWooAddUpdate" onchange="scheduleAutoSave()">
            <span style="color:#c4b5fd">افزودن/آپدیت/حذف ووکامرس (فقط تغییرات)</span>
          </label>
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:calc(11px * var(--font-scale,1))">
            <input type="checkbox" id="profileSyncBslAddUpdate" onchange="scheduleAutoSave()">
            <span style="color:#67e8f9">افزودن/آپدیت/بایگانی باسلام</span>
          </label>
        </div>
        <div id="profileSyncStatus" style="font-size:calc(10px * var(--font-scale,1));color:var(--text-dim);margin-top:6px"></div>
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border);display:flex;align-items:center;gap:8px">
          <label style="font-size:calc(11px * var(--font-scale,1));color:var(--text-dim)">🌐 اتصال این پروفایل:</label>
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:calc(11px * var(--font-scale,1))">
            <input type="checkbox" id="profileNetIndirect" onchange="scheduleAutoSave()">
            <span>غیرمستقیم (Worker/پروکسی)</span>
          </label>
          <span id="profileNetIndirectHint" style="font-size:calc(9px * var(--font-scale,1));color:var(--text-muted)"></span>
        </div>
      </div>
    </div>
    ...
    <div class="glass-card">
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          <label class="form-label">مقدار صفحه‌بندی:</label>
          <input type="text" class="form-control" id="pagVal" value="page" placeholder="page یا {page} یا سلکتور">
        </div>
      </div>
      ...
      <div class="btn-grid-actions">
        ...
        <button class="btn btn-secondary" onclick="startManualSync()">🔄 همگام‌سازی دستی (استخراج→ارسال)</button>
        <button class="btn btn-danger" onclick="stopActiveTasks()">⏹ توقف همه</button>
      </div>
      <div id="manualSyncProgress" style="display:none;margin-top:10px">
        <div style="height:6px;background:rgba(255,255,255,0.1);border-radius:4px;overflow:hidden">
          <div id="msBar" style="height:100%;width:0%;background:var(--primary);transition:width 0.3s"></div>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:calc(10.5px * var(--font-scale,1));color:var(--text-dim);margin-top:4px">
          <span id="msPhase">در انتظار...</span>
          <span id="msState" class="chip chip-primary">آماده</span>
        </div>
        <div id="msLog" style="max-height:120px;overflow-y:auto;font-size:calc(10px * var(--font-scale,1));margin-top:6px"></div>
  ...
  <main id="tab-settings" class="view-panel">
    <div class="glass-card">
      ...
      <div class="sub-tabs">
        ...
        <button class="sub-tab-btn" onclick="switchSettingsSub('system')">💻 نسخه PHP/Python</button>
        <button class="sub-tab-btn" onclick="switchSettingsSub('deployer')">📦 دیپلوی خودکار</button>
      </div>
      ...
      <div id="set-sub-system" class="settings-sub-panel" style="display:none;">
        <div class="glass-card" style="margin:0">
          <div class="card-header">
            <div class="card-title"><span class="card-title-icon">💻</span><span>نسخه PHP / Python و اطلاعات سیستم</span></div>
            <button class="btn btn-secondary btn-sm" onclick="loadSystemInfo()">🔄 تازه‌سازی</button>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:10px">
              <div style="font-size:calc(11px * var(--font-scale,1));color:var(--text-muted)">نسخه برنامه</div>
              <div id="sysAppVersion" style="font-weight:800;color:var(--primary)">—</div>
            </div>
            <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:10px">
              <div style="font-size:calc(11px * var(--font-scale,1));color:var(--text-muted)">Python</div>
              <div id="sysPythonVersion" style="font-weight:800;direction:ltr">—</div>
            </div>
            <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:10px">
              <div style="font-size:calc(11px * var(--font-scale,1));color:var(--text-muted)">Flask</div>
              <div id="sysFlaskVersion" style="font-weight:800;direction:ltr">—</div>
            </div>
            <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:10px">
              <div style="font-size:calc(11px * var(--font-scale,1));color:var(--text-muted)">PHP (اگر نصب باشد)</div>
              <div id="sysPhpVersion" style="font-weight:800;direction:ltr">—</div>
            </div>
          </div>
          <div style="margin-top:10px;background:var(--card);border:1px solid var(--border);border-radius:8px;padding:10px;white-space:pre-wrap;font-size:calc(11px * var(--font-scale,1));line-height:1.8" id="sysInfoBox">در حال بارگذاری...</div>
          <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btn-secondary btn-sm" onclick="checkSystemHealth()">🩺 تست سلامت سیستم</button>
            <button class="btn btn-secondary btn-sm" onclick="exportSystemInfo()">📋 کپی اطلاعات</button>
          </div>
        </div>
      </div>

      <!-- Deployer Auto -->
      <div id="set-sub-deployer" class="settings-sub-panel" style="display:none;">
        <div class="glass-card" style="margin:0">
          <div class="card-header">
            <div class="card-title"><span class="card-title-icon">📦</span><span>دیپلوی خودکار — جستجوی همه برنچ‌ها</span></div>
            <span class="chip chip-primary" id="deployerChip">آماده</span>
          </div>
          <p style="font-size:calc(11px * var(--font-scale,1));color:var(--text-dim);line-height:1.9;margin-bottom:10px">
            این دیپلوی خودکار همه برنچ‌های ریپو را (از جمله برنچ‌های arena و zip) اسکن می‌کند،
            <b>جدیدترین فایل</b> بر اساس <code>APP_VERSION</code> را پیدا کرده و اتوماتیک نصب می‌کند.
            مثل <code>deployer.py --auto</code> ولی از داخل مرورگر.
          </p>
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
            <a class="btn btn-secondary btn-sm" href="/deployer" target="_blank">↗ باز کردن آدرس جداگانهٔ دیپلوی (/deployer) — کار هم‌زمان با چند برنچ</a>
            <a class="btn btn-secondary btn-sm" href="/deployer.py" target="_blank" title="دانلود فایل مستقل deployer.py">📄 deployer.py</a>
          </div>
          <div class="form-grid">
            <div class="form-group">
              <label class="form-label">مخزن:</label>
              <input type="text" class="form-control" id="deployerRepo" value="fazilatma/amphp" dir="ltr">
            </div>
            <div class="form-group">
              <label class="form-label">فیلتر برنچ (خالی = همه):</label>
              <input type="text" class="form-control" id="deployerFilter" placeholder="arena, zip, ...">
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
            <button class="btn btn-secondary btn-sm" id="deployerScanBtn" onclick="deployerScan()">🔍 اسکن همه برنچ‌ها</button>
            <button class="btn btn-accent btn-sm" onclick="deployerAuto()">🚀 نصب خودکار جدیدترین</button>
            <button class="btn btn-secondary btn-sm" onclick="deployerInstallForce()">⚡ نصب اجباری جدیدترین</button>
          </div>

          <!-- Branch picker with version -->
          <div style="margin-top:14px; padding:12px; background:var(--card); border:1px solid var(--border); border-radius:var(--radius-md);">
            <div style="font-weight:800; font-size:calc(12px * var(--font-scale,1)); margin-bottom:8px;">🎯 انتخاب دستی برنچ برای نصب</div>
            <div class="form-grid">
              <div class="form-group" style="grid-column: span 2;">
                <label class="form-label">برنچ + نسخه:</label>
                <select class="form-control" id="deployerBranchSelect" onchange="deployerOnBranchChange()">
                  <option value="">— ابتدا اسکن کنید —</option>
                </select>
                <div id="deployerBranchMeta" style="margin-top:6px; font-size:calc(11px * var(--font-scale,1)); color:var(--text-dim);"></div>
              </div>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px;">
              <button class="btn btn-accent btn-sm" onclick="deployerInstallSelected()">⬇️ نصب برنچ انتخاب‌شده</button>
              <button class="btn btn-secondary btn-sm" onclick="deployerInstallSelected(true)">⚡ نصب اجباری انتخاب‌شده</button>
            </div>
          </div>

          <div id="deployerStatus" style="margin-top:12px;font-size:calc(11.5px * var(--font-scale,1));color:var(--text-dim);white-space:pre-wrap"></div>
          <div id="deployerTable" style="margin-top:10px;max-height:420px;overflow:auto"></div>
        </div>
      </div>
...
function switchSettingsSub(sub) {
  ...
  const subNames = ['woo', 'bsl', 'ai', 'network', 'msg', 'deploy', 'system', 'deployer'];
  const sIdx = subNames.indexOf(sub);
  if (sIdx !== -1 && btns[sIdx]) btns[sIdx].classList.add('active');
  if (sub === 'system') loadSystemInfo();
  if (sub === 'deployer') deployerScan();
}

/* ==================== START TAB — PROFILE SYNC & MANUAL SYNC ==================== */
function updateSyncStatusText() {
  const en = _el('profileSyncEn');
  const interval = _el('profileSyncInterval');
  const target = _el('profileSyncTarget');
  const txt = _el('profileSyncStatus');
  if (!txt) return;
  if (!en || !en.checked) { txt.textContent = 'همگام‌سازی غیرفعال است'; return; }
  const iv = interval ? interval.value : '15m';
  const tg = target ? target.value : 'woo';
  const tgFa = {woo:'ووکامرس', bsl:'باسلام', both:'هر دو'}[tg] || tg;
  txt.textContent = `فعال · هر ${iv} · مقصد: ${tgFa}`;
}

function syncToggleNoExtractBox() {
  const box = _el('profileNoExtractBox');
  const en = _el('profileSyncEn');
  if (!box || !en) return;
  box.style.display = en.checked ? 'block' : 'none';
}

let manualSyncTaskId = null;
let manualSyncTimer = null;

async function startManualSync() {
  const bar = _el('msBar');
  const phase = _el('msPhase');
  const state = _el('msState');
  const log = _el('msLog');
  const report = _el('msReport');
  const btn = _el('msFire');
  if (!btn) return;
  btn.disabled = true;
  if (bar) { bar.style.width = '5%'; bar.textContent = '۵٪'; }
  if (phase) phase.textContent = 'شروع';
  if (state) state.textContent = 'در حال ارسال درخواست...';
  if (log) log.innerHTML = '';
  if (report) report.innerHTML = '';
  const payload = {
    target: (_el('profileSyncTarget') ? _el('profileSyncTarget').value : 'woo'),
    no_extract: !!(_el('profileNoExtract') && _el('profileNoExtract').checked),
    refresh: !!(_el('profileSyncRefresh') && _el('profileSyncRefresh').checked),
    woo_add_update: !!(_el('profileWooAddUpdate') && _el('profileWooAddUpdate').checked),
    bsl_add_update: !!(_el('profileBslAddUpdate') && _el('profileBslAddUpdate').checked),
    net_indirect: !!(_el('profileNetIndirect') && _el('profileNetIndirect').checked)
  };
  try {
    const res = await fetch('/api/sync-matrix/start', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const json = await res.json();
    if (!json.ok || !json.task_id) { showToast(json.error || 'شروع ناموفق بود', 'error'); btn.disabled = false; return; }
    manualSyncTaskId = json.task_id;
    showToast('همگام‌سازی دستی آغاز شد', 'success');
    if (state) state.textContent = 'در حال اجرا...';
    msWatch();
  } catch(e) {
...
async function msWatch() {
  if (!manualSyncTaskId) return;
  try {
    const res = await fetch('/api/tasks/' + encodeURIComponent(manualSyncTaskId));
    const json = await res.json();
    if (!json.ok || !json.task) { setTimeout(msWatch, 1500); return; }
    const t = json.task;
    const bar = _el('msBar');
    const phase = _el('msPhase');
    const state = _el('msState');
    ...
    const report = _el('msReport');
    const btn = _el('msFire');
    if (bar) { const pct = Math.max(5, t.progress || 5); bar.style.width = pct + '%'; bar.textContent = toFa(pct) + '٪'; }
    if (phase) phase.textContent = t.step || '';
    if (state) state.textContent = t.status === 'running' ? 'در حال اجرا...' : t.status;
    if (log && Array.isArray(t.logs)) {
      const tail = t.logs.slice(-40);
      log.innerHTML = tail.map(l => `<div>${escHtml(l)}</div>`).join('');
      log.scrollTop = log.scrollHeight;
    }
    if (t.status === 'completed' || t.status === 'failed' || t.status === 'cancelled') {
      if (btn) btn.disabled = false;
      if (report) {
        if (t.error) report.innerHTML = `<div class="chip chip-danger">❌ ${escHtml(t.error)}</div>`;
        else report.innerHTML = `<div class="chip chip-success">✅ پایان: ${escHtml(t.step || '')} — ${toFa((t.result && t.result.synced_items) || 0)} مورد</div>`;
      }
      if (state) state.textContent = t.status === 'completed' ? 'پایان ✅' : ('وضعیت: ' + t.status);
      if (bar && t.status === 'completed') { bar.style.width = '100%'; bar.textContent = '۱۰۰٪'; }
      manualSyncTaskId = null;
      loadTasks();
      return;
    }
    manualSyncTimer = setTimeout(msWatch, 1200);
  } catch(e) {
    manualSyncTimer = setTimeout(msWatch, 2000);
  }
}

/* ==================== SYSTEM INFO (PHP version parity) ==================== */
async function loadSystemInfo() {
  const appV = _el('sysAppVersion');
  const pyV = _el('sysPythonVersion');
  const flaskV = _el('sysFlaskVersion');
  const phpV = _el('sysPhpVersion');
  const box = _el('sysInfoBox');
  if (box) box.textContent = 'در حال بارگذاری...';
  try {
    const res = await fetch('/api/system/info');
    const json = await res.json();
    if (!json.ok) { if (box) box.textContent = 'خطا: ' + (json.error || ''); return; }
    const info = json.info || {};
    if (appV) appV.textContent = info.app_version || '';
    if (pyV) pyV.textContent = `${info.python_impl || ''} ${info.python_version || ''}`.trim();
    if (flaskV) flaskV.textContent = info.flask_version || '—';
    if (phpV) phpV.textContent = info.php_version || 'نصب نیست';
    if (box) {
      const disk = info.disk || {};
      const files = (info.app_files || []).map(f => `${f.name} (${f.size_kb}KB)`).join('\\n');
      box.textContent = `Platform: ${info.platform || ''}\\n` +
        `Python: ${info.python_impl || ''} ${info.python_version || ''}\\n` +
        `Flask: ${info.flask_version || ''}\\n` +
        `PHP: ${info.php_version || ''} ${info.php_sapi ? '('+info.php_sapi+')' : ''}\\n` +
        `Disk: ${disk.used_mb || '?'}MB used / ${disk.total_mb || '?'}MB total (free ${disk.free_mb || '?'}MB)\\n` +
        `Home: ${disk.home || ''}\\n` +
        `Data file: ${info.env ? info.env.SCRAPER_DATA_FILE : ''}\\n` +
        `PWD: ${info.env ? info.env.PWD : ''}\\n` +
        `Files:\\n${files}`;
    }
  } catch(e) {
    if (box) box.textContent = 'خطای شبکه: ' + e.message;
  }
}

async function checkSystemHealth() {
  showToast('بررسی سلامت سیستم...', 'info');
  try {
    const res = await fetch('/health');
    const json = await res.json();
    showToast(`سالم ✅ نسخه ${json.version || ''} — ${toFa(json.tasks_running || 0)} وظیفه فعال`, 'success');
    const box = _el('sysInfoBox');
...
function exportSystemInfo() {
  ...
  if (!text) { showToast('اطلاعاتی برای کپی نیست', 'info'); return; }
  navigator.clipboard.writeText(text).then(() => showToast('اطلاعات سیستم کپی شد', 'success')).catch(() => {
...
/* ==================== DEPLOYER AUTO (search all branches) ==================== */
let deployerLastScan = null;

async function deployerScan() {
  const repo = (_el('deployerRepo') ? _el('deployerRepo').value.trim() : '') || 'fazilatma/amphp';
  const filter = _el('deployerFilter') ? _el('deployerFilter').value.trim() : 'zip,arena,deploy';
  const status = _el('deployerStatus');
  const table = _el('deployerTable');
  const btn = _el('deployerScanBtn');
  const sel = _el('deployerBranchSelect');
  if (btn) btn.disabled = true;
  if (status) status.textContent = '⏳ در حال اسکن همهٔ برنچ‌ها...';
  if (table) table.innerHTML = '<div class="chip chip-primary">در حال بارگذاری...</div>';
  if (sel) sel.innerHTML = '<option value="">— در حال اسکن... —</option>';
  try {
    const res = await fetch('/api/deployer/scan', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ repo, filter }) });
    const json = await res.json();
    if (!json.ok) {
      if (status) status.textContent = '❌ ' + (json.error || 'خطا');
      if (table) table.innerHTML = `<div class="chip chip-danger">${escHtml(json.error || 'خطا')}</div>`;
      if (sel) sel.innerHTML = '<option value="">— خطا —</option>';
      return;
    }
    deployerLastScan = json;
    const newest = json.newest;
    const local = json.local_version || '';
    if (status) {
      if (newest) {
        const cmp = (JSON.stringify(newest.version_tuple || []) > JSON.stringify(json.local_tuple || [])) ? 'جدیدتر موجود است ⬆️' : 'به‌روز هستید ✅';
        status.innerHTML = `محلی: <b>${escHtml(local)}</b> | جدیدترین: <b>${escHtml(newest.version)} (${escHtml(newest.branch)})</b> — ${cmp} | ${toFa(json.count)} برنچ دارای فایل`;
      } else {
        status.textContent = `هیچ برنچی با scraper4.py معتبر پیدا نشد (${toFa(json.count)} بررسی)`;
      }
    }
    const rows = json.branches || [];
    // Populate select dropdown with branch + version
    if (sel) {
      if (!rows.length) sel.innerHTML = '<option value="">— نتیجه‌ای نیست —</option>';
      else {
        sel.innerHTML = rows.map(r => {
          const isNewest = newest && r.branch === newest.branch;
          const label = `${r.branch} — v${r.version}${isNewest ? ' ⭐ جدیدترین' : ''} (${Math.round(r.size/1024)}KB)`;
          return `<option value="${escHtml(r.branch)}" data-version="${escHtml(r.version)}" data-size="${r.size}" data-sha="${escHtml(r.sha||'')}" ${isNewest ? 'selected' : ''}>${escHtml(label)}</option>`;
        }).join('');
        deployerOnBranchChange();
      }
    }
    if (table) {
      if (!rows.length) table.innerHTML = '<div class="chip">نتیجه‌ای نیست</div>';
      else {
        table.innerHTML = rows.map(r => {
          const isNewest = newest && r.branch === newest.branch;
          return `<div class="ai-model-row${isNewest ? ' is-master' : ''}" style="display:flex; justify-content:space-between; align-items:center; gap:8px; flex-wrap:wrap;">
            <div style="flex:1; min-width:200px;">
              <b dir="ltr">${escHtml(r.branch)}</b>
              <span dir="ltr" style="color:var(--primary); font-weight:800; margin-inline-start:6px;">v${escHtml(r.version)}</span>
              ${isNewest ? '<span class="chip chip-success" style="margin-inline-start:6px;">جدیدترین ⭐</span>' : ''}
              <div style="margin-top:4px; display:flex; gap:4px; flex-wrap:wrap;">
                <span class="chip">${toFa(Math.round(r.size/1024))}KB</span>
                ${r.sha ? `<span class="chip" dir="ltr">${escHtml(r.sha)}</span>` : ''}
                <span class="chip chip-primary" dir="ltr">${escHtml((r.version_tuple||[]).join('.'))}</span>
              </div>
            </div>
            <div style="display:flex; gap:6px;">
              <button class="btn btn-secondary btn-sm" onclick="deployerSelectBranch('${escHtml(r.branch).replace(/'/g, '&#39;')}')">👁 انتخاب</button>
              <button class="btn btn-accent btn-sm" onclick="deployerInstall('${escHtml(r.branch).replace(/'/g, '&#39;')}')">⬇️ نصب</button>
            </div>
          </div>`;
        }).join('');
      }
    ...
    if (status) status.textContent = 'خطای شبکه: ' + e.message;
  } finally {
    if (btn) btn.disabled = false;
  }
}

function deployerSelectBranch(branch) {
  const sel = _el('deployerBranchSelect');
  if (sel) { sel.value = branch; deployerOnBranchChange(); sel.scrollIntoView({behavior:'smooth', block:'center'}); }
}

function deployerOnBranchChange() {
  const sel = _el('deployerBranchSelect');
  const meta = _el('deployerBranchMeta');
  if (!sel || !meta) return;
  ...
  const sha = opt.getAttribute('data-sha') || '';
  meta.innerHTML = `نسخه: <b dir="ltr" style="color:var(--primary)">v${escHtml(ver)}</b> | حجم: ${toFa(Math.round(parseInt(size||'0')/1024))}KB ${sha ? '| SHA: <span dir="ltr">'+escHtml(sha)+'</span>' : ''}`;
}

async function deployerInstallSelected(force) {
  const sel = _el('deployerBranchSelect');
  if (!sel || !sel.value) { showToast('ابتدا یک برنچ را انتخاب کنید', 'info'); return; }
  const branch = sel.value;
  const repo = (_el('deployerRepo') ? _el('deployerRepo').value.trim() : '') || 'fazilatma/amphp';
  const status = _el('deployerStatus');
  if (!confirm(`برنچ «${branch}»${force ? ' به صورت اجباری' : ''} نصب شود؟`)) return;
  if (status) status.textContent = `⏳ نصب ${branch}...`;
  try {
    const res = await fetch('/api/deployer/install', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ repo, branch, force: !!force }) });
    const json = await res.json();
    if (json.ok && json.changed) {
      showToast(json.message || 'نصب شد', 'success');
      if (status) status.innerHTML = `✅ ${escHtml(json.message)}<br>محلی: ${escHtml(json.local_version||'')} → ${escHtml(json.remote_version||'')} (${escHtml(json.branch||'')})`;
      setTimeout(() => location.reload(), 2000);
    } else if (json.ok) {
      showToast(json.message || '', 'info');
      if (status) status.textContent = json.message || '';
    } else {
      showToast(json.error || 'خطا', 'error');
      if (status) status.textContent = '❌ ' + (json.error || '');
    }
  } catch(e) { showToast('خطای شبکه: ' + e.message, 'error'); if (status) status.textContent = 'خطای شبکه: ' + e.message; }
}

async function deployerAuto() {
  const repo = (_el('deployerRepo') ? _el('deployerRepo').value.trim() : '') || 'fazilatma/amphp';
  const filter = _el('deployerFilter') ? _el('deployerFilter').value.trim() : 'zip,arena,deploy';
  if (!confirm('اسکن همهٔ برنچ‌ها و نصب خودکار جدیدترین نسخه؟')) return;
  const status = _el('deployerStatus');
  if (status) status.textContent = '⏳ نصب خودکار جدیدترین نسخه...';
  try {
    const res = await fetch('/api/deployer/install', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ repo, filter }) });
    const json = await res.json();
    if (json.ok) {
      if (json.changed) {
        showToast(json.message || 'نصب شد', 'success');
        ...
        showToast(json.message || 'به‌روز هستید', 'info');
        if (status) status.textContent = json.message || '';
      }
    } else {
      showToast(json.error || 'نصب ناموفق', 'error');
      if (status) status.textContent = '❌ ' + (json.error || '');
    }
  } catch(e) { showToast('خطای شبکه: ' + e.message, 'error'); if (status) status.textContent = 'خطای شبکه: ' + e.message; }
}

async function deployerInstall(branch) {
  if (!branch) return;
  if (!confirm(`برنچ «${branch}» نصب شود؟`)) return;
  const repo = (_el('deployerRepo') ? _el('deployerRepo').value.trim() : '') || 'fazilatma/amphp';
  const status = _el('deployerStatus');
  if (status) status.textContent = `⏳ نصب ${branch}...`;
  try {
    const res = await fetch('/api/deployer/install', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ repo, branch }) });
    const json = await res.json();
    if (json.ok && json.changed) {
      showToast(json.message || 'نصب شد', 'success');
      if (status) status.textContent = json.message;
...
async function deployerInstallForce() {
  const repo = (_el('deployerRepo') ? _el('deployerRepo').value.trim() : '') || 'fazilatma/amphp';
  const filter = _el('deployerFilter') ? _el('deployerFilter').value.trim() : 'zip,arena,deploy';
  if (!confirm('نصب اجباری جدیدترین نسخه حتی اگر نسخه محلی جدیدتر باشد؟')) return;
  const status = _el('deployerStatus');
  if (status) status.textContent = '⏳ نصب اجباری...';
  try {
    const res = await fetch('/api/deployer/install', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ repo, filter, force:true }) });
    const json = await res.json();
    if (json.ok && json.changed) { showToast(json.message || 'نصب شد', 'success'); setTimeout(() => location.reload(), 2000); }
    else showToast(json.message || json.error || '', json.ok ? 'info' : 'error');
...
function renderProfileOptions(cfg) {
  ...
  fill('topProfileSelect');
  fill('profileSelect');
  const nameEl = _el('profileName');
  if (nameEl && !nameEl.value) nameEl.value = active;
  // update sync status if exists
  if (_el('profileSyncStatus')) updateSyncStatusText();
  if (_el('profileNoExtractBox')) syncToggleNoExtractBox();
}
