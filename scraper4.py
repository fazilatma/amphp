...
def scrape(config: dict[str, Any]) -> ScrapeReport:
    ...
    if enrich and report.products:
        ...
        for position,product in enumerate(candidates,1):
            ...
            attempted+=1
            try:
                if task_id:live_task_update(task_id,88+round(attempted/max(1,min(len(candidates),detail_limit))*9),f"جزئیات محصول {attempted} از {min(len(candidates),detail_limit)}","running",clean_text(product.get("title"))[:160],done=position,total=min(len(candidates),detail_limit),extracted=len(report.products))
...
def scrape_live_worker(task_id: str, config: dict[str,Any]) -> None:
    ...
    try:
        live_task_update(task_id,2,"اعتبارسنجی آدرس و تنظیمات","running","استخراج مستقل و موازی روی سرور آغاز شد؛ وظایف دیگر منتظر این منبع نمی‌مانند",execution="parallel");config=dict(config);config["job_id"]=task_id;config["_live_task_id"]=task_id;report=scrape(config);live_task_update(task_id,98,"ذخیره نتایج روی سرور","running",f"{len(report.products)} محصول آماده شد");result=scrape_result(report,clean_text(config.get("_profile_name")));live_task_update(task_id,100,"استخراج کامل شد","completed",f"{result['total']} محصول از {result['pages']} صفحه",result=result,done=result['pages'],total=result['pages'],extracted=result['total'],eta_seconds=0)
    except Exception as exc:
