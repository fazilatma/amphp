    ...
    if enrich and report.products:
        ...
        for product in list(report.products.values()):
            ...
            if not detail_selectors and product.get("image") and product.get("price") and product.get("images"):
                continue
            try:
                detail_engine=clean_text(config.get("fetch_engine","auto")).lower();detail_engine=detail_engine if detail_engine in {"requests","cloudscraper","curl_cffi"} else clean_text(diag.get("engine")) or "requests"
                detail=render_playwright(product["link"],fetcher.timeout,1) if detail_engine=="playwright" else fetcher.get(product["link"],referer=source,engine=detail_engine)
...
def scrape_result(report: ScrapeReport, profile_name: str="") -> dict[str,Any]:
    ...
    with DATA_LOCK:
        ...
        for key,product in current.items():
            ...
            old=previous[key];price_changed=str(product.get("price",""))!=str(old.get("price",""));other_changed=any(str(product.get(f,""))!=str(old.get(f,"")) for f in ("stock","image","title","category"))
            if price_changed:row=dict(product);row["previous_price"]=old.get("price","");price_rows.append(row)
            if other_changed:row=dict(product);row["previous_title"]=old.get("title","");row["previous_stock"]=old.get("stock","");changed_rows.append(row)
...
body{font-size:16px;line-height:1.72}small,.section-head small,.provider-row small,.task-top small,.ai-compare-card small{font-size:13px!important;line-height:1.65}label{font-size:14px!important;margin-bottom:8px}input,select,textarea,button,.file-btn{font-size:16px!important;line-height:1.5}button,.file-btn{min-height:46px}.card h3{font-size:20px}.hero h1{font-size:24px}.sub{font-size:14px}.tabs button,.admin-nav button,.ai-tabs button{font-size:13px!important}.tabs button{min-height:64px}.note,.status{font-size:15px}.badge,.model-chip,.latency-pill,.task-metrics,.task-event,.operation-item small,.answer-cell,.answer-cell summary,.live-step,.live-details,.clickable-counts button span,.space-card span{font-size:13px!important}th{font-size:14px}td{font-size:15px}.clickable-counts button b,.space-card b{font-size:25px}.comparison-history{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:9px}.history-item{padding:12px;border:1px solid #334155;border-radius:12px;background:#0f172a}.history-item b{display:block;color:#7dd3fc;margin-bottom:6px}.history-item span{display:inline-block;margin:2px 0 2px 8px;color:#cbd5e1}.history-item.current{border-color:#34d39966;background:linear-gradient(145deg,#052e2a,#10243c)}
...
<section id="results" class="pane"><div class="card result-hero"><div><h3>📊 محصولات استخراج‌شده</h3><small>جدول کامل محصولات فقط در این تب نمایش داده می‌شود.</small></div><span id="resultCountBadge" class="badge">۰ محصول</span></div><div class="card comparison-history-card"><div class="section-head"><div><h3>🕘 تاریخچه مقایسه پروفایل</h3><small>خلاصه ده استخراج اخیر روی سرور نگهداری می‌شود.</small></div></div><div id="comparisonHistory" class="comparison-history"><div class="note">پس از اولین استخراج، تاریخچه اینجا نمایش داده می‌شود.</div></div></div><div class="primary-actions actions"><button class="green" onclick="downloadCSV()">↓ CSV</button><button class="gray" onclick="downloadJSON()">↓ JSON</button><button class="gray" onclick="downloadXLSX()">↓ Excel</button></div><div id="resultsMount"></div></section>
...
const taskKindLabel=k=>({scrape:'استخراج DOM',profile_dispatch:'ارسال مقصدها',basalam_sdk:'SDK باسلام',ai_test:'آزمایش مدل‌های AI'}[k]||k||'—');
...
function openChangeList(kind){let rows=kind==='all'?products:(lastComparison.lists?.[kind]||[]);$('changeModalTitle').textContent=changeLabels[kind]||'جزئیات';$('changeModalSub').textContent=rows.length+' محصول';$('changeModalList').innerHTML=rows.map((p,i)=>`<article class="change-product">${p.image?`<img src="${esc(p.image)}" loading="lazy" alt="">`:'<span></span>'}<div><b>${esc(p.title||'بدون عنوان')}</b><br><small>SKU: ${esc(p.sku||'—')} · موجودی: ${esc(p.stock??'—')}</small></div><div>${kind==='price_changed'?`<span class="price-before">${esc(p.previous_price||'—')}</span> ← <span class="price-after">${esc(p.price||'—')}</span>`:`<span class="price-after">${esc(p.price||'—')}</span>`}${p.link?`<br><a href="${esc(p.link)}" target="_blank" rel="noopener">مشاهده محصول ↗</a>`:''}</div></article>`).join('')||'<div class="note">محصولی در این گروه وجود ندارد.</div>';$('changeModal').classList.add('open');document.body.style.overflow='hidden'}
