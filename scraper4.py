...
def api_live_tasks():
    ...
    return jsonify(ok=True,tasks=result,summary={"running":sum(x.get("status") in {"waiting","running"} for x in result),"completed":sum(x.get("status")=="completed" for x in result),"failed":sum(x.get("status")=="failed" for x in result)})
...
def scrape_live_worker(task_id: str, config: dict[str,Any]) -> None:
    try:
        live_task_update(task_id,2,"اعتبارسنجی آدرس و تنظیمات","running","وظیفه استخراج در سرور آغاز شد");config=dict(config);config["job_id"]=task_id;config["_live_task_id"]=task_id;report=scrape(config);live_task_update(task_id,92,"ذخیره نتایج روی سرور","running",f"{len(report.products)} محصول آماده شد");result=scrape_result(report);live_task_update(task_id,100,"استخراج کامل شد","completed",f"{result['total']} محصول از {result['pages']} صفحه",result=result)
...
def api_scrape_start():
    ...
    task=live_task_create("scrape","استخراج محصولات",private=False);threading.Thread(target=scrape_live_worker,args=(task["id"],config),name="scrape-live",daemon=True).start();return jsonify(ok=True,task=task)
...
const taskStatusLabel=s=>({waiting:'در صف',running:'در حال اجرا',completed:'کامل',failed:'ناموفق',cancelled:'متوقف'}[s]||s||'—');
...
async function wooTest(){const target=$('wooConnectionStatus')||$('wooStatus');try{target.textContent='در حال تست…';let d=await deployApi('/api/woo/test',{method:'POST',body:'{}'});target.innerHTML='<span class="ok">اتصال موفق است.</span>'}catch(e){target.innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
