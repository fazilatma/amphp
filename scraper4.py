...
function compactTaskDetails(details){const out=[];for(const x of details||[]){const text=String(x.text||'');const k=text.replace(/\d+/g,'#').slice(0,100);if(out.length&&out[out.length-1].k===k){out[out.length-1].n++;continue}out.push({at:x.at,text,n:1,k})}return out}
