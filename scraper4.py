...
def basalam_send_one_sdk(product: dict[str,Any]) -> dict[str,Any]:
    ...
    client=basalam_client();sku=clean_text(product.get("sku"));existing_id=int(product.get("_destination_id") or 0) if cfg.get("update_existing",True) else 0
    if cfg.get("update_existing",True) and not existing_id and sku:
...
def basalam_send_one_api(product: dict[str,Any]) -> dict[str,Any]:
    ...
    if not vendor or not category:raise ValueError("شناسه غرفه و دسته‌بندی پیش‌فرض باسلام لازم است")
    existing_id=int(product.get("_destination_id") or 0) if cfg.get("update_existing",True) else 0
    existing=None
    if cfg.get("update_existing",True) and not existing_id and sku:
...
def profile_dispatch_worker(task_id: str, profile_name: str, products: list[dict[str,Any]], destinations: list[str], woo_status: str, woo_update: bool, profile_rules: dict[str,Any]) -> None:
        ...
        for destination in destinations:
            ...
            for index,product in enumerate(products,1):
                ...
                try:
                    ...
                    if destination=="woocommerce":result=woo_send_one(destination_product,woo_status,woo_update)
...
.reconcile-card{border-color:#38bdf855}.reconcile-report{margin-top:10px}.reconcile-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}.reconcile-stats button{padding:10px 5px;background:#0a1729;border:1px solid #334155}.reconcile-stats b{display:block;font-size:20px}.reconcile-list{max-height:260px;overflow:auto;margin-top:9px}.reconcile-item{padding:8px;border-bottom:1px solid #263951;font-size:11px}.reconcile-item small{display:block;color:#94a3b8}@media(max-width:640px){.reconcile-stats{grid-template-columns:repeat(2,1fr)}}
...
<header class="hero"><div class="hero-main"><div class="logo">🕸️</div><div><div class="eyebrow">مرکز استخراج محصول</div><h1>🛒 اسکرپر <small id="appVersion">v4.4.0</small></h1><div class="sub">استخراج و مدیریت هوشمند محصولات</div></div></div><div class="hero-badge"><span>●</span> آنلاین و آماده</div></header>
...
const taskKindLabel=k=>({scrape:'استخراج DOM',detail_extract:'جزئیات محصولات',profile_dispatch:'ارسال مقصدها',destination_reconcile:'مغایرت‌گیری مقصد',basalam_sdk:'SDK باسلام',ai_test:'آزمایش مدل‌های AI'}[k]||k||'—');
