...
def api_deploy_dependencies():
    ...
    if not python_bin: return jsonify(ok=False, error="Python virtualenv پیدا نشد؛ نصب‌کننده کامل را اجرا کنید"), 500
...
def ai_test_public(job: dict[str,Any]) -> dict[str,Any]:
    ...
    return {"id":job.get("id"),"status":job.get("status"),"created_at":job.get("created_at"),"cursor":job.get("cursor",0),"total":len(rows),"ok_count":sum(x.get("status")=="ok" for x in rows),"failed_count":sum(x.get("status")=="failed" for x in rows),"waiting":len(rows)-len(done),"options":job.get("options",{}),"rows":rows[-300:]}
...
def api_ai_test_start():
    if not deploy_authorized():return deploy_auth_error()
    body=request.get_json(silent=True) or {};per=max(1,min(500,int(body.get("per_provider",50))));only=bool(body.get("only_untested",False));skip=body.get("skip_nonchat",True) is not False;rows=[];data=load_data();providers=normalize_ai_providers(data.get("ai_providers",{}))
    ...
    jid="ait-"+time.strftime("%Y%m%d-%H%M%S")+"-"+secrets.token_hex(2);job={"id":jid,"status":"waiting","created_at":int(time.time()),"cursor":0,"rows":rows,"options":{"prompt":clean_text(body.get("prompt") or "سلام؛ فقط کلمه سالم را پاسخ بده"),"per_provider":per,"only_untested":only,"skip_nonchat":skip,"delay_ms":max(0,min(60000,int(body.get("delay_ms",120))))}}
...
def api_ai_test_process(job_id: str):
    ...
    while int(job.get("cursor",0))<len(job["rows"]) and processed<batch:
        ...
        try:
            answer=ai_chat(job["options"]["prompt"],row["provider"],row["model"]);row.update(status="ok",answer=clean_text(answer)[:500],error="",latency_ms=round((time.monotonic()-started)*1000))
        except Exception as exc:row.update(status="failed",answer="",error=clean_text(exc)[:500],latency_ms=round((time.monotonic()-started)*1000))
        # Persist health on the model just like PHP's tested/available metadata.
        ...
        if model is not None:model.update({"tested":True,"available":row["status"]=="ok","latencyMs":row["latency_ms"],"lastTestAt":int(time.time()),"testError":row.get("error","")})
...
def basalam_sdk_status() -> dict[str,Any]:
    try:return {"installed":True,"version":importlib.metadata.version("basalam-sdk"),"python":sys.executable}
    except importlib.metadata.PackageNotFoundError:return {"installed":False,"version":"","python":sys.executable}


def ensure_basalam_sdk() -> bool:
    """Install the official SDK into the web app's own Python environment."""
    ...
    try:
        if basalam_sdk_status()["installed"]:return False
        cmd=[sys.executable,"-m","pip","install","--disable-pip-version-check","--no-cache-dir","basalam-sdk"]
        if sys.prefix==getattr(sys,"base_prefix",sys.prefix):cmd.insert(-1,"--user")
        result=subprocess.run(cmd,capture_output=True,text=True,timeout=240,env={**os.environ,"PIP_NO_CACHE_DIR":"1"})
        importlib.invalidate_caches()
        if result.returncode or not basalam_sdk_status()["installed"]:
            detail=clean_text((result.stderr or result.stdout)[-1200:])
            raise ValueError("نصب خودکار SDK رسمی باسلام ناموفق بود: "+detail)
        return True
...
async function testBasalam(){try{$('bslAdminStatus').innerHTML='<span class="spinner"></span> بررسی و در صورت نیاز نصب خودکار SDK رسمی…';let d=await deployApi('/api/basalam/test',{method:'POST',body:'{}'});$('bslAdminStatus').innerHTML='<span class="ok">اتصال SDK موفق: '+esc(d.user)+(d.installed_now?' · SDK همین حالا خودکار نصب شد':'')+'</span>';$('bslSdkBadge').textContent='SDK '+(d.sdk?.version||'آماده');$('bslSdkBadge').className='badge ok'}catch(e){$('bslAdminStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
...
let activeAITestJob='';
function renderAITest(job){if(!job){$('aiTestSummary').innerHTML='';$('aiTestRows').innerHTML='<div class="note">هنوز صف آزمونی ساخته نشده است.</div>';return}activeAITestJob=job.id;$('aiTestSummary').innerHTML=`<div class="space-card"><b>${job.cursor}/${job.total}</b><span>پیشرفت</span></div><div class="space-card"><b>${job.ok_count}</b><span>سالم</span></div><div class="space-card"><b>${job.failed_count}</b><span>ناموفق</span></div>`;$('aiTestRows').innerHTML=(job.rows||[]).slice().reverse().map(r=>`<div class="test-row ${esc(r.status)}"><b>${esc(r.provider_name)} · ${esc(r.model_name)}</b><span class="badge">${r.status==='ok'?'✓ سالم':r.status==='failed'?'✕ خطا':'در انتظار'}</span><small>${r.latency_ms?r.latency_ms+' ms · ':''}${esc(r.answer||r.error||'')}</small></div>`).join('')}
...
async function startAITests(){try{let body={per_provider:+$('ai_test_per').value,delay_ms:+$('ai_test_delay').value,only_untested:$('ai_test_only').checked,skip_nonchat:$('ai_test_skip').checked,prompt:$('ai_health_prompt').value};let d=await deployApi('/api/ai/test/jobs',{method:'POST',body:JSON.stringify(body)});renderAITest(d.job);$('aiStatus').innerHTML='<span class="ok">صف سلامت با '+d.job.total+' مدل ساخته شد؛ مرحله بعد را اجرا کنید.</span>'}catch(e){$('aiStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
async function processAITests(){if(!activeAITestJob){await loadAITestJobs();if(!activeAITestJob)return}try{$('aiStatus').textContent='در حال آزمایش مرحله‌ای مدل‌ها…';let d=await deployApi('/api/ai/test/jobs/'+encodeURIComponent(activeAITestJob)+'/process',{method:'POST',body:JSON.stringify({batch:+$('ai_test_batch').value})});renderAITest(d.job);$('aiStatus').innerHTML='<span class="ok">'+d.processed+' آزمون اجرا شد؛ '+d.job.waiting+' مورد باقی مانده.</span>';if(d.job.status==='completed')await loadAI()}catch(e){$('aiStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>'}}
