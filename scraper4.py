...
function compactTaskDetails(details){const out=[];for(const x of details||[]){const text=String(x.text||'');let k=text;if(/باسلام/.test(text)&&(/API error 400/.test(text)||/HTTP 400/.test(text)||/Field required/.test(text)))k='basalam-400';else if(/باسلام/.test(text)&&/HTTP 403/.test(text))k='basalam-403';else k=text.replace(/\d+/g,'#').slice(0,90);if(out.length&&out[out.length-1].k===k){out[out.length-1].n++;continue}out.push({at:x.at,text,n:1,k})}return out}
