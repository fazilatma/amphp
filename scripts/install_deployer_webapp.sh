...
# NOTE: DEPLOYER_WEB_TOKEN is deliberately NOT written here. The deployer web
# app reads the token from deployer_token.txt next to the target, so editing
# that file takes effect without reinstalling. (web_token is only validated
# when the script generated/wrote the token file.)
lines = [
